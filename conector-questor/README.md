...
Operações permitidas: teste do nWeb, leitura dos parâmetros de relatórios, apuração PIS/Cofins, cancelamentos, a Conferência de Saídas (`nFisRRResumoConfLctoFisSai`) e a importação de locações por meio da Conferência de Saídas (Gráfico), filtrada por espécie `REC`. O token fica somente neste computador.
