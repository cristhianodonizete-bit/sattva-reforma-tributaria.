...
Telas.dados = async (el) => {
  ...
  const luzProntidao = (id, destino) => {
    ...
    return `<button type="button" class="btn pq ${verde ? 'vazio' : 'perigo'}" data-prontidao="${id}" data-prontidao-destino="${destino || id}"><span aria-hidden="true">${verde ? '●' : '●'}</span> ${verde ? 'LIBERADO' : 'PENDÊNCIAS'}</button>`;
  };
...
Telas.periodoAnalisado = async (el) => {
  ...
  const render = async () => {
    ...
    el.innerHTML = cab('CENTRAL DE DADOS · CONTROLE DA ANÁLISE', `Período analisado <span class="tag ${estado?.status==='VERDE'?'c':'a'}">● ${estado?.status==='VERDE'?'LIBERADO':'PENDÊNCIAS'}</span>`, 'Defina uma única janela de competências para importações, conferência de completude e relatórios desta empresa.', `<button class="btn" id="editarPeriodoAnalisado">${periodo ? 'Alterar período' : 'Definir período'}</button>`) + resumo;
