  ...
  const render = async () => {
    ...
    el.querySelector('#novaAnalisePlanejamento').onclick = () => {
      ...
      A.modal({ titulo:'Nova análise tributária', descricao:'A fotografia dos dados será congelada. Premissas não sobrescrevem dados reais.', largura:760,
        corpo:`${A.campo('titulo','Título','Planejamento tributário')}${A.area('descricao','Objetivo / observação','',2)}<div class="aviso"><b>Dados que serão solicitados dentro da análise</b><br>Simples: PGDAS 2024 e 2025. Lucro Presumido/Real: faturamento mensal dos últimos 24 meses. Todos: folha e pró-labore dos últimos 12 meses. Sem histórico suficiente, a projeção será linear e exibirá alerta de risco.</div><label class="campo"><span>Empresas da análise</span><div class="check-list">${empresas.map((x,i)=>`<label><input type="checkbox" name="empresa_${x.id}" ${i===0?'checked':''}> ${A.esc(x.razao_social)}</label>`).join('')}</div></label>`,
    ...
    el.querySelector('[data-importar-planejamento]')?.addEventListener('click', ()=>A.modal({titulo:'Importar dados para o planejamento',descricao:'Os dados entram na base da empresa com validação existente e serão usados em uma nova fotografia.',largura:720,corpo:`${A.selecao('empresa_id','Empresa',detalhe.empresas.map(x=>({v:x.id,t:x.razao_social})),detalhe.empresas[0]?.id)}${A.selecao('tipo','Tipo de arquivo',[{v:'pgdas',t:'PGDAS (Simples)'},{v:'folha',t:'Folha e pró-labore'},{v:'receita',t:'Faturamento mensal'}],'folha')}<label class="campo"><span>Arquivo XLSX, XLS ou CSV</span><input type="file" id="arquivoPlanejamento" accept=".xlsx,.xls,.csv"></label>`,confirmar:'Importar',aoConfirmar:async(f,modal)=>{const arq=modal.querySelector('#arquivoPlanejamento')?.files?.[0];if(!arq)throw new Error('Selecione o arquivo.');const fd=new FormData();fd.append('arquivo',arq);const rota=f.tipo==='pgdas'?`/empresas/${f.empresa_id}/importar/pgdas`:f.tipo==='folha'?`/empresas/${f.empresa_id}/importar/folhas-pagamento`:`/empresas/${f.empresa_id}/importar/receitas-sem-dfe`;await A.api(rota,{metodo:'POST',corpo:fd});A.toast('Importação concluída. Crie uma nova fotografia para atualizar a análise.','ok');}}));
...
function detalhePlanejamento(d, resultados, recomendado) {
  ...
  const necessidades=(snap?.dados?.necessidades_coleta||[]).flatMap(x=>x.itens.filter(i=>!i.recebido).map(i=>`${x.empresa}: ${i.titulo}`)); const alerta=(snap?.dados?.necessidades_coleta||[]).map(x=>x.alerta).filter(Boolean)[0];
  return `<p class="mini">${empresas.map((x)=>A.esc(x.razao_social)).join(' · ')}</p><div class="grade g3">${A.kpi('Fotografia',snap?`v${snap.versao}`:'PENDENTE',snap?'dados congelados':'não disponível')}${A.kpi('Decisão',A.esc(a.status),'não altera cadastro')}${A.kpi('Recomendação',recomendado?A.esc(recomendado.cenario):'VALIDAÇÃO NECESSÁRIA',recomendado?'cenário completo':'dados insuficientes')}</div><div class="acoes-topo" style="margin-top:14px"><button class="btn" data-executar-planejamento>Executar cenários</button><button class="btn vazio" data-importar-planejamento>Importar dados</button><button class="btn vazio" data-fotografia-planejamento>Atualizar fotografia</button><button class="btn vazio" data-cnaes-planejamento>Consultar CNAEs</button><button class="btn vazio" data-premissa-planejamento>Premissas de margem</button>${linhas.length?'<button class="btn vazio" data-assistente-planejamento>Assistente Tributário</button>':''}${a.status==='EM_REVISAO'?'<button class="btn ouro" data-aprovar-planejamento>Aprovar decisão</button>':''}</div>${necessidades.length?`<div class="aviso atencao" style="margin-top:14px"><b>Informações solicitadas</b><br>${necessidades.map(A.esc).join('<br>')}${alerta?`<br><br><b>Atenção:</b> ${A.esc(alerta)}`:''}</div>`:''}${linhas.length?A.tabela([{t:'Cenário',r:x=>A.esc(x.cenario.replaceAll('_',' '))},{t:'Tributos',num:true,r:x=>x.tributos===null?'INDICATIVO':A.moeda(x.tributos)},{t:'Carga efetiva',num:true,r:x=>x.carga===null?'—':A.pct(x.carga)},{t:'Δ baseline',num:true,r:x=>x.economia===null?'—':A.moeda(x.economia)},{t:'Confiança',r:x=>`<span class="tag ${x.confianca==='ALTA'?'c':x.confianca==='MEDIA'?'b':'n'}">${A.esc(x.confianca)}</span>`},{t:'Pendências',r:x=>String(x.pendencias)}],linhas):'<div class="aviso">Execute a análise para comparar cada empresa e o consolidado do grupo.</div>'}<h3 style="margin-top:16px">Premissas e histórico</h3><p class="mini">${(d.premissas||[]).length} premissa(s) · ${(d.eventos||[]).length} evento(s) auditável(is). Dados reais, projetados e manuais permanecem identificados.</p>`;
}
})();
