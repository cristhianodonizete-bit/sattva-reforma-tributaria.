...
Telas.empresas = async (el) => {
  ...
  document.getElementById('novaEmpresa').onclick = () => {
    const modal = A.modal({ titulo: 'Cadastrar empresa', descricao: 'Ao informar o CNPJ, a consulta cadastral traz CNAE e QSA para conferência. Nada é salvo até a confirmação.',
      corpo: form({}, true), aoConfirmar: async (d) => { const r=await A.api('/empresas', { metodo:'POST', corpo:d }); A.toast(r.enriquecimento_qsa?.status==='CONSULTADO_NA_CRIACAO' ? `Empresa cadastrada com ${r.enriquecimento_qsa.socios_recuperados} sócio(s) consultado(s).` : 'Empresa cadastrada', 'ok'); await A.carregarEmpresas(); A.ir('empresas'); },
    });
    const cnpjInput = modal.fundo.querySelector('[name="cnpj"]'); const previa = modal.fundo.querySelector('#preconsultaCadastro'); let ultimo = '';
    const consultar = async () => {
      const cnpj=String(cnpjInput.value||'').replace(/\D/g,''); if(cnpj.length!==14 || cnpj===ultimo) return; ultimo=cnpj;
      ...
      try {
        ...
        ['razao_social','nome_fantasia','uf','municipio','cnae','atividade','cnaes_secundarios','data_abertura'].forEach((campo)=>{const input=modal.fundo.querySelector(`[name="${campo}"]`);if(input&&c[campo])input.value=c[campo];});
...
Telas.dados = async (el) => {
  ...
  const fonteApuracao = (registro) => {
    ...
    if (tipo === 'RELATORIO_ERP' || versao.includes('QUESTOR') || /^QUESTOR\b/i.test(String(registro.nome_original || ''))) return 'Questor';
    if (tipo === 'PDF') return 'PDF';
    if (['XLSX', 'XLS', 'CSV'].includes(tipo)) return 'Planilha';
  ...
  const periodoDoPerfil = periodoResposta?.periodo || null;
  const destinoPgdas = (documento) => {
    const competencia = String(documento.competencia_detectada || '');
    const dentroDoPerfil = competencia && (!periodoDoPerfil || (competencia >= periodoDoPerfil.competencia_inicio && competencia <= periodoDoPerfil.competencia_fim));
    if (dentroDoPerfil) return `<span class="tag c">Perfil + planejamento</span><div class="mini">Compõe a janela atual do Perfil Tributário e permanece no histórico do planejamento.</div>`;
    return `<span class="tag b">Planejamento Tributário</span><div class="mini">Fora da janela atual do Perfil; será preservado para a análise histórica e as projeções.</div>`;
  };
  ...
  const historicoIndisponivelApuracoes = itemProntidao('apuracoes')?.declaracoes_historico || [];
  const luzProntidao = (id, destino) => {
    const p=itemProntidao(id); const verde=p?.status==='VERDE';
    return `<button type="button" class="prontidao-luz ${verde ? 'liberado' : 'pendente'}" data-prontidao="${id}" data-prontidao-destino="${destino || id}"><span aria-hidden="true">●</span> ${verde ? 'LIBERADO' : 'PENDÊNCIAS'}</button>`;
  };
  const painelProntidao = `<section class="cartao" style="margin-top:16px"><div class="cabecalho-lista"><div><div class="olho">PRONTIDÃO DAS ENTREGAS</div><h2>O que falta para o sistema entregar</h2><p class="desc">Verde: resolvido por dado ou declaração auditável. Vermelho: exige ação. Clique no status para ver a pendência.</p></div>${luzProntidao('periodo','periodoAnalisado')}</div><div class="grade g3" style="margin-top:12px">${(prontidao.etapas || []).map((p)=>`<div class="cartao" style="box-shadow:none;border:1px solid var(--linha)"><div style="display:flex;justify-content:space-between;gap:8px;align-items:center"><b>${A.esc(p.titulo)}</b>${luzProntidao(p.id,p.id)}</div><p class="mini">${p.status==='VERDE'?'Dados resolvidos para esta etapa.':A.esc(p.pendencias?.[0] || 'Há informações a resolver.')}</p></div>`).join('')}</div></section>`;

  const etapasCentrais = {
  ...
  el.innerHTML = topoEtapa +
    ...
    <div class="cartao" data-documentos-central-painel="importacao" data-importacao-planilha-painel="cadastro" style="margin-top:16px" id="cadastro">
      ...
      <div class="cartao" data-documentos-central-painel="importacao" data-importacao-planilha-painel="movimentacao" style="margin-top:16px" id="movimentacao">
        ...
        <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap">
          ...
          <button class="btn ouro pq" id="executarMotorPlanilha">Ver prontidão do motor</button>
          <span class="mini" style="margin-left:auto;align-self:center">${total.c} lançamentos · ${A.moeda(total.v)}</span>
        </div>
      </div>
      <div class="cartao" data-documentos-central-painel="importacao" data-importacao-planilha-painel="conferencia" style="margin-top:16px">
    ...
    <div class="cartao" style="margin-top:16px"><h2>Receitas registradas</h2><p class="desc">Cada lançamento mostra o valor, a origem e a evidência que o trouxe para a base. As importações do Questor permanecem rastreáveis pelo número do lançamento.</p>${A.tabela([
      ...
      {t:'Descrição',r:x=>A.esc(x.descricao || '—')},
      {t:'Valor',num:true,r:x=>`<b>${A.moeda(x.valor)}</b>`},
      {t:'Fonte e evidência',r:x=>`<span class="tag ${String(x.origem||'').startsWith('QUESTOR') ? 'c' : ''}">${A.esc(String(x.origem||'MANUAL').startsWith('QUESTOR') ? 'Questor' : x.origem || 'Manual')}</span><div class="mini">${A.esc(x.evidencia || 'Sem evidência adicional')}</div>${x.identificador_origem ? `<div class="mini mono">Lançamento ${A.esc(x.identificador_origem)}</div>` : ''}`},
  ...
  ${grupoCentral === 'apuracoes' ? `<div class="cartao" style="margin-top:16px"><div style="display:flex;gap:8px;flex-wrap:wrap">${simplesNacional ? '<button class="btn" id="centralPgdas">Importar PGDAS</button><button class="btn vazio" id="centralIntegra">Integra Contador</button><button class="btn vazio" id="testarIntegraPgdas" hidden>Diagnosticar acesso Integra</button><button class="btn vazio" id="baixarPgdasIntegra" hidden>Baixar PGDAS — Integra Contador</button><button class="btn vazio" id="verJsonIntegra" hidden>Ver JSON de uma competência</button><button class="btn vazio" id="verLogsIntegra" hidden>Ver últimas consultas</button><button class="btn vazio" onclick="App.baixarArquivo(\'/modelos/pgdas\').catch(e=>App.toast(e.message,\'erro\'))">Baixar modelo PGDAS</button>' : `<button class="btn" id="centralApuracao">Enviar apuração PIS/Cofins</button>`}<button class="btn vazio" id="declararHistoricoApuracao">Declarar início posterior/inatividade</button></div>${historicoIndisponivelApuracoes.length ? `<div class="aviso atencao" style="margin-top:14px"><b>Histórico não disponível declarado:</b> ${A.esc(historicoIndisponivelApuracoes.join(', '))}. Essas competências não tiveram dados baixados/importados; foram registradas como ausência declarada e não representam apuração.</div>` : ''}<div class="grade g2" style="margin-top:16px">${A.kpi('Documentos enviados',simplesNacional ? (pgdasResposta.documentos || []).length : apuracoesImportadas.length,'histórico de apurações')}${A.kpi('Pendências',(itemProntidao('apuracoes')?.pendencias || []).length,'competências a resolver')}</div><div class="aviso ${simplesNacional ? 'bom' : 'atencao'}" style="margin-top:14px">${simplesNacional ? 'Empresa do Simples Nacional: planilha XLSX, XLS ou CSV é lida de forma estruturada. PDF digital do PGDAS é lido localmente e exige revisão antes de entrar no histórico.' : exigeApuracaoPisCofins ? 'Empresa no Lucro Presumido ou Lucro Real: envie a apuração histórica de PIS/Cofins.' : 'A apuração aplicável depende do regime cadastrado da empresa.'}</div>${!simplesNacional ? `<div style="margin-top:18px"><h3>Importações de apuração</h3><p class="mini">Cada competência preserva o documento e a origem utilizada na análise.</p>${A.tabela([{t:'Competência',r:x=>`<b>${A.esc(x.competencia || 'Não identificada')}</b>`},{t:'Fonte',r:x=>`<span class="tag ${x.fonte_importacao === 'Questor' ? 'c' : ''}">${A.esc(x.fonte_importacao)}</span>`},{t:'Documento / relatório',r:x=>A.esc(x.nome_original || 'Sem nome')},{t:'Importado em',r:x=>A.esc(x.importado_em || '—')},{t:'Situação',r:x=>A.esc(x.status_validacao || x.status_processamento || 'INDETERMINADO')}],apuracoesImportadas,{vazio:'Nenhuma apuração importada ainda. Use PDF, planilha ou a integração Questor.'})}</div>` : ''}${simplesNacional && (pgdasResposta.documentos || []).length ? `<div style="margin-top:14px"><h3>Documentos PGDAS em revisão</h3><p class="mini">O destino não muda a validação: somente documentos confirmados entram nos cálculos. A competência fora da janela atual fica preservada para o Planejamento Tributário.</p>${A.tabela([{t:'Arquivo',r:x=>A.esc(x.nome_original)},{t:'Competência',r:x=>A.esc(x.competencia_detectada || 'Não identificada')},{t:'Destino de uso',r:x=>destinoPgdas(x)},{t:'Fonte',r:x=>String(x.tipo_documento || '').startsWith('INTEGRA_CONTADOR')?'Integra Contador / PDF oficial':String(x.metodo_extracao || '').includes('AZURE')?'Azure / PDF':'Arquivo enviado'},{t:'Situação',r:x=>A.esc(x.status_processamento)},{t:'',r:x=>{const ref=A.esc(x.hash_sha256 || x.id);return `<button class="btn pq vazio" data-pgdas-revisar="${ref}">Revisar</button><button class="btn pq vazio" data-pgdas-reprocessar="${ref}">Reprocessar</button>${x.status_processamento === 'VALIDADO_USUARIO' ? '<span class="mini"> Confirmado</span>' : `<button class="btn pq" data-pgdas-confirmar="${ref}">Confirmar dados</button>`}`;}}],pgdasResposta.documentos)}</div>` : ''}</div>` : ''}
    ...
    // A navegação documental precisa aparecer antes dos painéis. Movemos a
    // barra já renderizada para logo abaixo do título sem duplicar controles.
    ...
    el.querySelectorAll('[data-prontidao]').forEach((botao) => botao.addEventListener('click', () => {
      ...
      const abrir=()=>{ if (destino==='periodoAnalisado') A.ir('periodoAnalisado'); else if (destino==='empresa') A.ir('empresas'); else { S.aba.centralDados=destino; A.ir('dados'); } };
      A.modal({ titulo:`${p.status==='VERDE'?'Liberado':'Pendências'} — ${p.titulo}`, descricao:p.status==='VERDE'?'Esta etapa está resolvida para a empresa selecionada.':'Resolva cada item abaixo para tornar a etapa verde.', corpo:p.status==='VERDE'?'<div class="aviso bom">Requisito atendido por dado preenchido, importado ou declaração auditável.</div>':`<div class="aviso alto"><ul>${(p.pendencias||[]).map((x)=>`<li>${A.esc(x)}</li>`).join('')}</ul></div><p class="mini">Se não houve operação, a ausência pode ser declarada com justificativa — sem inventar dados fiscais.</p>`, confirmar:'Abrir etapa', aoConfirmar:async()=>abrir() });
    }));
    ...
    document.getElementById('centralIntegra')?.addEventListener('click', () => A.modal({
      ...
      aoConfirmar:async({ acao }) => {
        const botoes = { lote:'baixarApuracoesPeriodo', diagnostico:'testarIntegraPgdas', consultas:'verLogsIntegra', individual:'baixarApuracaoVigente', json:'verJsonIntegra' };
        document.getElementById(botoes[acao])?.click();
      },
    }));
    ...
    document.getElementById('verJsonIntegra')?.addEventListener('click', () => A.modal({
      titulo:'JSON de diagnóstico — Integra Contador',
      descricao:'Consulta somente leitura de uma competência. O retorno não é armazenado no histórico.',
      corpo:A.campo('competencia','Competência (AAAA-MM)','','text','placeholder="2026-06"'),
      confirmar:'Consultar JSON',
      aoConfirmar: async (dados) => {
        const r = await A.api(`/empresas/${S.empresaId}/integra-contador/pgdas/diagnostico-json`, { metodo:'POST', corpo:dados });
        A.modal({ titulo:`JSON Serpro — ${r.competencia}`, largura:1100, confirmar:'Fechar',
          descricao:'Resposta integral da consulta de índice. Ela permite identificar a chamada de detalhe para declaração e DAS.',
          corpo:`<pre style="max-height:62vh;overflow:auto;padding:14px;background:#091b2c;color:#dbeafe;border-radius:8px;white-space:pre-wrap;word-break:break-word">${A.esc(JSON.stringify(r.retorno_serpro, null, 2))}</pre>`,
          aoConfirmar: async () => {} });
      },
    }));
    document.getElementById('verJsonIntegra')?.insertAdjacentHTML('afterend', '<button class="btn vazio" id="baixarApuracaoVigente" hidden>Baixar apuração vigente</button><button class="btn vazio" id="baixarApuracoesPeriodo" hidden>Baixar todas do período</button>');
    ...
    document.getElementById('baixarApuracoesPeriodo')?.addEventListener('click', async () => {
      const { periodo, janela_apuracao:janelaApuracao } = await A.api(`/empresas/${S.empresaId}/periodo-analisado`);
      const janela = janelaApuracao || periodo;
      if (!janela?.competencia_inicio || !janela?.competencia_fim) { A.toast('Defina o período analisado antes de baixar as apurações.', 'erro'); return; }
      A.modal({ titulo:'Baixar apurações PGDAS-D do período', descricao:`Serão consultadas as últimas declarações transmitidas da janela de apurações configurada: ${janela.competencia_inicio} a ${janela.competencia_fim}, uma competência por vez. Cada PDF será salvo para revisão e nada entrará automaticamente no Perfil Tributário.`, confirmar:'Baixar todas', corpo:`<div class="aviso atencao">${janela.meses || 'As'} competência(s) serão consultadas no Serpro e cada chamada pode ser cobrada. O sistema continuará as demais mesmo que uma competência não tenha declaração.</div>`, aoConfirmar:async()=>{
        const competencias=[]; let cursor=new Date(`${janela.competencia_inicio}-01T12:00:00`); const limite=new Date(`${janela.competencia_fim}-01T12:00:00`);
    ...
    document.getElementById('verLogsIntegra')?.addEventListener('click', async () => {
      try {
        ...
        A.modal({ titulo:'Últimas consultas — Integra Contador', largura:1050, confirmar:'Fechar', corpo:A.tabela([
          {t:'Data',r:x=>A.esc(x.criado_em || '—')},{t:'Ano',r:x=>A.esc(x.ano_calendario)},{t:'Solicitadas',r:x=>A.esc(x.competencias_solicitadas || '[]')},{t:'Reconhecidas',r:x=>A.esc(x.competencias_encontradas || '[]')},{t:'Situação',r:x=>`<span class="tag ${x.status === 'CONCLUIDA' ? 'c' : 'a'}">${A.esc(x.status)}</span>`},{t:'Diagnóstico',r:x=>A.esc(x.mensagem || '—')},
        ],r.logs || [],{vazio:'Nenhuma consulta registrada para esta empresa.'}) });
      } catch (e) { A.toast(e.message || 'Não foi possível carregar o histórico de consultas.', 'erro'); }
    });
    document.getElementById('baixarPgdasIntegra')?.addEventListener('click', async () => {
      ...
      if (!cfg.configurado) {
        A.modal({ titulo:'Integra Contador não configurado', descricao:'A conexão é configurada apenas no ambiente seguro do servidor.', confirmar:'Fechar', corpo:`<div class="aviso atencao"><b>O que falta</b><br>${A.esc(cfg.mensagem || 'Credenciais não configuradas.')}<br><br><span class="mini">No Render, cadastre INTEGRA_CONTADOR_BASE_URL e uma credencial fornecida pelo Serpro/provedor: INTEGRA_CONTADOR_ACCESS_TOKEN, INTEGRA_CONTADOR_API_KEY ou INTEGRA_CONTADOR_TOKEN_URL + INTEGRA_CONTADOR_CLIENT_ID + INTEGRA_CONTADOR_CLIENT_SECRET. As chaves não aparecem nesta tela e não são gravadas no banco.</span></div>` });
        return;
      }
      A.confirmar('Consultar somente as declarações PGDAS-D já transmitidas no período analisado? O retorno ficará em revisão e nada será transmitido ou alterado na Receita.', async () => {
        ...
        if (!quantidade) {
          const detalhes = (r.diagnosticos || []).map((x) => `${x.ano_calendario}: ${x.objetos_analisados} objeto(s), ${x.objetos_com_competencia} com competência, ${x.objetos_com_das} com DAS`).join(' · ');
          A.toast(`Nenhuma declaração PGDAS-D foi reconhecida. Nada foi importado.${detalhes ? ` Retorno analisado: ${detalhes}.` : ''}`, 'erro');
        }
    ...
    el.querySelectorAll('[data-pgdas-revisar]').forEach((b) => { b.onclick = () => {
      const doc = (pgdasResposta.documentos || []).find((x) => String(x.hash_sha256 || x.id) === String(b.dataset.pgdasRevisar));
      if (!doc) return;
      ...
      const auditoria = blocos.length ? `<h3>Validação tributária por bloco</h3>${A.tabela([
        ...
        {t:'Simples Nacional',r:x=>`Nominal ${A.pct(x.calculation?.aliquota_nominal || 0)} · dedução ${A.moeda(x.calculation?.parcela_deduzir || 0)}<br><span class="mini">Alíquota efetiva do Simples: ${A.pct(x.calculation?.aliquota_efetiva_simples || 0)}</span>`},
        {t:'PIS',r:x=>`<span class="mini">Repartição do PIS: ${A.pct(x.calculation?.pis_distribution_percentage || 0)}<br>Alíquota efetiva do PIS: ${A.pct(x.calculation?.pis_effective_rate || 0)}</span><br>PGDAS ${A.moeda(x.pgdas_validation?.pgdas_pis || 0)} · calculado ${A.moeda(x.pgdas_validation?.calculated_pis || 0)}<br><span class="mini">diferença ${A.moeda(x.pgdas_validation?.pis_difference || 0)} · ${x.pgdas_validation?.pis_match ? 'confere' : 'revisar'}</span>`},
        {t:'COFINS',r:x=>`<span class="mini">Repartição da COFINS: ${A.pct(x.calculation?.cofins_distribution_percentage || 0)}<br>Alíquota efetiva da COFINS: ${A.pct(x.calculation?.cofins_effective_rate || 0)}</span><br>PGDAS ${A.moeda(x.pgdas_validation?.pgdas_cofins || 0)} · calculado ${A.moeda(x.pgdas_validation?.calculated_cofins || 0)}<br><span class="mini">diferença ${A.moeda(x.pgdas_validation?.cofins_difference || 0)} · ${x.pgdas_validation?.cofins_match ? 'confere' : 'revisar'}</span>`},
    ...
    el.querySelectorAll('[data-pgdas-reprocessar]').forEach((b) => { b.onclick = () => A.confirmar('Reler este PDF PGDAS já armazenado com as regras atuais? Isso não fará nova consulta ao Integra Contador.', async () => {
      ...
      A.toast('PGDAS reprocessado sem nova consulta ao Serpro. Revise e confirme os campos.', 'ok'); A.ir('dados');
    }); });
    ...
    if (botoesReprocessamentoPgdas.length) {
      ...
      el.querySelector('#reprocessarPgdasSelecionados')?.addEventListener('click',()=>A.confirmar('Reler os PDFs PGDAS selecionados com as regras atuais? Não haverá nova consulta ao Integra Contador.',async()=>{
        const ids=selecionados(),resposta=await A.api(`/empresas/${S.empresaId}/pgdas/documentos/reprocessar-lote`,{metodo:'POST',corpo:{documentos:ids}}),falhas=(resposta.resultados||[]).filter((x)=>!x.ok);
        if(falhas.length){A.modal({titulo:'Reprocessamento PGDAS com falhas',confirmar:'Fechar',corpo:`<div class="aviso atencao"><b>${ids.length-falhas.length} reprocessado(s); ${falhas.length} falha(s).</b><br><br>${falhas.map((x)=>`${A.esc(x.competencia||x.referencia)} — ${A.esc(x.erro||'Falha não detalhada')}`).join('<br>')}</div>`});}else {A.toast(`${ids.length} documento(s) reprocessado(s) sem consulta ao Serpro.`,'ok');A.ir('dados');}
      }));
  ...
  el.querySelectorAll('[data-excluir-receita]').forEach((botao) => { botao.onclick = () => {
    const receita = (dadosAdicionais.receitas_sem_dfe || []).find((x) => Number(x.id) === Number(botao.dataset.excluirReceita));
    if (!receita) return;
    const identificacao = receita.identificador_origem ? `lançamento ${receita.identificador_origem}` : `receita de ${A.moeda(receita.valor)}`;
    A.confirmar(`Excluir ${identificacao}? A remoção alcança a fonte compartilhada e este lançamento deixará de compor as análises.`, async () => {
      await A.api(`/empresas/${S.empresaId}/receitas-sem-dfe/${receita.id}`, { metodo: 'DELETE' });
      A.toast('Lançamento excluído da base e da fonte compartilhada.', 'ok');
      A.ir('dados');
    });
  }; });
...
Telas.perfilLegado = async (el) => {
  ...
  el.innerHTML = cab('Módulo 1.a', 'Perfil Tributário e Raio-X Histórico', 'Leitura da carga atual de PIS/Cofins por competência. A CBS aparece apenas na comparação do Raio-X.', '<button class="btn vazio" id="centralDadosPerfil">Central de Dados</button><button class="btn vazio" id="complementarCbs">Completar informações</button><button class="btn" id="atualizarCbs">Atualizar Perfil CBS</button>') +
    ...
    <div class="cartao" style="margin-top:16px"><h2>Apurações Históricas de PIS/Cofins</h2><p class="mini">O documento original é preservado. Campos sem evidência aparecem como “Não identificado”; baixa confiança exige revisão humana.</p>${apuracoes.length ? apuracoes.map((a) => `<div class="cartao" style="margin-top:12px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center"><div><b>${A.esc(a.nome_original)}</b><div class="mini">Competência: ${A.esc(a.competencia || 'Não identificada')} · processamento: ${A.esc(a.status_processamento || 'INDETERMINADO')}</div></div><div><button class="btn pq vazio" data-apuracao-revisar="${a.id}">Revisar</button><button class="btn pq vazio" data-apuracao-reprocessar="${a.id}">Reprocessar</button><button class="btn pq perigo" data-apuracao-excluir="${a.id}">Excluir</button>${a.status_validacao === 'VALIDADO_USUARIO' ? '<span class="mini"> Confirmado</span>' : `<button class="btn pq" data-apuracao-confirmar="${a.id}">Confirmar dados</button>`}</div></div>${a.campos_pendentes?.length ? `<div class="aviso atencao" style="margin-top:10px"><b>Pendências/baixa confiança:</b> ${a.campos_pendentes.map(A.esc).join(', ')}</div>` : ''}</div>`).join('') : A.vazio('Nenhuma apuração histórica enviada', 'Envie o documento pela Central de Dados para extrair PIS/Cofins sem recalcular CBS.', '<button class="btn" id="centralDadosApuracao">Ir para Central de Dados</button>')}</div>
  ...
  el.querySelectorAll('[data-apuracao-excluir]').forEach((b) => { b.onclick = () => A.confirmar('Excluir este documento de apuração e seus campos extraídos? Depois você poderá enviar o mesmo arquivo novamente.', async () => { await A.api(`/empresas/${S.empresaId}/apuracoes-pis-cofins/${b.dataset.apuracaoExcluir}`, { metodo:'DELETE' }); A.toast('Apuração excluída. Você já pode reenviar o arquivo.', 'ok'); A.ir('perfil'); }); });
...
Telas.perfil = async (el) => {
  ...
  const memoriaMensalPgdas = [...composicaoPisCofinsPgdas.map((x) => ({ ...x, escopo:'ATUAL' })), ...composicaoPisCofinsPgdasHistorico.map((x) => ({ ...x, escopo:'HISTORICO' }))]
    ...
    .reduce((mapa, x) => {
      const atual = mapa.get(x.competencia) || { competencia:x.competencia, escopo:x.escopo, receita_caixa:0, receita_competencia:null, receita_competencia_vinculada:null, receita_competencia_sem_anexo:null, pis_declarado:0, pis_competencia:null, cofins_declarado:0, cofins_competencia:null, status_competencia:'NAO_CALCULADO', motivo_competencia:null, exemplos_competencia:[], documentos_nao_associados:[], receitas_nao_associadas:[], memoria_calculo_competencia:[], blocos_pgdas:[], regime_caixa:false, validado:true };
      atual.receita_caixa += Number(x.receita_pgdas) || 0;
      if (atual.receita_competencia === null && x.receita_competencia_total !== null && x.receita_competencia_total !== undefined) atual.receita_competencia = Number(x.receita_competencia_total);
      if (atual.receita_competencia_vinculada === null && x.receita_competencia_vinculada !== null && x.receita_competencia_vinculada !== undefined) atual.receita_competencia_vinculada = Number(x.receita_competencia_vinculada);
      if (atual.receita_competencia_sem_anexo === null && x.receita_competencia_sem_anexo !== null && x.receita_competencia_sem_anexo !== undefined) atual.receita_competencia_sem_anexo = Number(x.receita_competencia_sem_anexo);
      ...
      if (atual.status_competencia === 'NAO_CALCULADO') {
        ...
        atual.receitas_nao_associadas = x.calculo_competencia_receitas_nao_associadas || [];
  ...
  const memoriaCompetencia = (linha) => {
    ...
    const naoAssociados=linha.documentos_nao_associados || [], receitasNaoAssociadas=linha.receitas_nao_associadas || [];
    ...
    ],[...regras.values()],{vazio:'Regra PGDAS indisponível.'})}</div>${naoAssociados.length ? `<div style="margin-top:10px"><b>${naoAssociados.length} documento(s) sem Anexo para o cálculo</b><p class="mini">Estes documentos compõem a cobertura da competência, mas não entram no PIS/Cofins recalculado até terem vínculo seguro.</p>${A.tabela([{t:'Documento',r:x=>`<b>${A.esc(x.documento)}</b><div class="mini">${A.esc(x.modelo)} · CFOP ${A.esc(x.cfop||'—')}</div>`},{t:'Descrição / chaves fiscais',r:x=>`${A.esc(x.descricao||'—')}<div class="mini">NCM ${A.esc(x.ncm||'—')} · NBS ${A.esc(x.nbs||'—')} · LC 116 ${A.esc(x.lc116||'—')}</div>`},{t:'Valor',num:true,r:x=>A.moeda(x.valor)},{t:'Motivo',r:x=>A.esc(x.motivo)}],naoAssociados)}</div>` : ''}${receitasNaoAssociadas.length ? `<div style="margin-top:10px"><b>${receitasNaoAssociadas.length} outra(s) receita(s) sem Anexo para o cálculo</b>${A.tabela([{t:'Receita',r:x=>`<b>${A.esc(x.descricao)}</b><div class="mini">${A.esc(x.origem||'—')} · segregação: ${A.esc(x.segregacao_apuracao||'não informada')}</div>`},{t:'Valor',num:true,r:x=>A.moeda(x.valor)},{t:'Motivo',r:x=>A.esc(x.motivo)}],receitasNaoAssociadas)}</div>` : ''}<p class="mini" style="margin:10px 0 0">Base total da competência: ${linha.receita_competencia === null ? 'não calculada' : A.moeda(linha.receita_competencia)}. Base vinculada a Anexo: ${linha.receita_competencia_vinculada === null ? '—' : A.moeda(linha.receita_competencia_vinculada)}. A base de caixa e os valores declarados vêm do PGDAS e não são alterados.</p></details>`;
  };
  ...
  const tabelaDetalheComposicao = (filtro) => A.tabela([
    {t:'Competência',r:x=>A.esc(rotuloCompetencia(x.competencia))},{t:'Modelo fiscal',r:x=>A.esc(String(x.modelo_fiscal||'—').toUpperCase())},{t:'CFOP',r:x=>`<span class="mono">${A.esc(x.cfop||'—')}</span>`},{t:'Decisão',r:x=>`<span class="tag ${x.compoe_receita?'c':'a'}">${x.compoe_receita?'Compõe receita':'Fora da receita'}</span><div class="mini">${A.esc(x.motivo||'')}</div>`},{t:'Tratamento PIS/Cofins',r:x=>A.esc(x.tratamento_pis_cofins||'Não informado na fonte')},{t:'Itens',num:true,r:x=>x.itens},{t:'Valor documental',num:true,r:x=>A.moeda(x.valor)}
  ...
  const conteudoComposicao = `<div class="cartao"><div class="cabecalho-lista"><div><h2>Composição da receita importada</h2><p class="desc">Faturamento mensal por modelo fiscal e outras receitas confirmadas. Receita monofásica, isenta ou com alíquota zero continua no total; quando a EFD-Contribuições trouxer o CST, ela fica segregada na auditoria.</p></div><span class="tag">${composicaoReceita.length + detalhesOutrasReceitas.length} grupo(s)</span></div><div class="abas" style="margin:16px 0 12px"><button class="ativo" data-filtro-composicao="receita">Operações que compõem receita</button><button data-filtro-composicao="geral">Todos os documentos</button></div><div id="tabelaComposicaoMensal">${tabelaComposicaoMensal('receita')}</div><details style="margin-top:16px"><summary><b>Auditoria por competência, modelo, CFOP e tratamento</b></summary><div id="tabelaDetalheComposicao" style="margin-top:12px">${tabelaDetalheComposicao('receita')}</div></details>${detalhesOutrasReceitas.length ? `<details style="margin-top:16px"><summary><b>Outras receitas que compõem o Perfil</b></summary><div style="margin-top:12px">${A.tabela([{t:'Competência',r:x=>A.esc(rotuloCompetencia(x.competencia))},{t:'Item',r:x=>`<b>${A.esc(x.tipo_receita || 'Outra receita')}</b><div class="mini">${A.esc(x.descricao || '')}</div>`},{t:'Fonte',r:x=>`<span class="tag c">${A.esc(String(x.origem || 'Manual').startsWith('QUESTOR') ? 'Questor' : x.origem || 'Manual')}</span>${x.identificador_origem ? `<div class="mini mono">Lançamento ${A.esc(x.identificador_origem)}</div>` : ''}`},{t:'Valor',num:true,r:x=>`<b>${A.moeda(x.valor)}</b>`}],detalhesOutrasReceitas)}</div></details>` : ''}</div>`;
  ...
  el.innerHTML = cab('Módulo 1.a · diagnóstico', 'Perfil Tributário',
    ...
    `<div class="cartao"><div class="cabecalho-lista"><div><h2>Resumo da apuração atual</h2><p class="desc">${usarRecalculoCompetencia ? `PIS/Cofins recalculados pela base integral da competência em ${recalculosCompetencia.size} competência(s). O PGDAS declarado permanece disponível para confronto.` : 'Valores efetivamente importados. A alíquota efetiva final é PIS/Cofins apurados ÷ receita analisada.'}</p></div><span class="tag">${A.esc(usarRecalculoCompetencia ? 'recalculado por competência' : origem)}</span></div>${avisoRecalculoIncompleto}
      ...
      ${chaveRegime === 'simples_nacional' && linhasMemoriaMensalPgdas.length ? `<div class="cartao" style="margin-top:16px;box-shadow:none"><div class="cabecalho-lista"><div><h3>Memória mensal: PGDAS de caixa × competência</h3><p class="mini">O declarado vem do PGDAS (caixa). O cálculo por competência usa somente os documentos fiscais ativos da mesma competência, classificados nos Anexos do PGDAS. Sem vínculo seguro, o sistema não rateia nem inventa um valor.</p></div></div>${A.tabela([
        { t:'Competência', r:x=>`<b>${A.esc(x.competencia)}</b><div class="mini">${x.escopo === 'ATUAL' ? 'Período atual' : 'Histórico fora do período'} · ${x.regime_caixa ? 'Regime de caixa' : 'Regime de competência'}</div>` },
        { t:'Receitas', num:true, r:x=>`PGDAS/caixa ${A.moeda(x.receita_caixa)}<br>Base da competência ${String(x.status_competencia||'').startsWith('CALCULADO') ? A.moeda(x.receita_competencia) : 'NÃO CALCULADA'}${x.receita_competencia_vinculada !== null ? `<br><span class="mini">Base distribuída nos Anexos ${A.moeda(x.receita_competencia_vinculada || 0)}</span>` : ''}` },
  ...
  el.querySelectorAll('[data-apuracao-excluir]').forEach((botao) => { botao.onclick = () => A.confirmar('Excluir este documento de apuração e seus campos extraídos? Depois você poderá enviar o mesmo arquivo novamente.', async () => {
    await A.api(`/empresas/${S.empresaId}/apuracoes-pis-cofins/${botao.dataset.apuracaoExcluir}`, { metodo:'DELETE' });
    A.toast('Apuração excluída. Você já pode reenviar o arquivo.', 'ok'); A.ir('perfil');
  }); });
};
...
Telas.mapaOperacional = async (el) => {
  ...
  const possuiSaidas = Boolean(d.escopo?.produto || d.escopo?.servico);
  const baseMapa = d.correlacoes || [];
  ...
  el.innerHTML = cab('DIAGNÓSTICO · MAPA OPERACIONAL', 'Possibilidades tributárias por atividade',
    ...
    `<section class="cartao"><div class="cabecalho-lista"><div><div class="olho">CNAE COM ESCOPO DOS DOCUMENTOS EMITIDOS</div><h2>${possuiSaidas ? 'Possibilidades tributárias compatíveis com a atividade' : 'Sem escopo de produto ou serviço nas saídas'}</h2><p class="desc">${possuiSaidas ? `A análise parte do CNAE. Os documentos emitidos limitaram esta consulta a: ${d.escopo?.produto ? 'produtos/NCM' : ''}${d.escopo?.produto && d.escopo?.servico ? ' e ' : ''}${d.escopo?.servico ? 'serviços/NBS + LC 116' : ''}.` : 'Os documentos foram concluídos, mas não há saída classificada como mercadoria ou serviço no período analisado.'}</p></div><span class="tag n">${linhas.length} linha(s)</span></div>
...
Telas.periodoAnalisado = async (el) => {
  ...
  const render = async () => {
    const [dados, prontidao] = await Promise.all([A.api(`/empresas/${S.empresaId}/periodo-analisado`), A.api(`/empresas/${S.empresaId}/prontidao-dados`)]);
    const periodo = dados.periodo;
    const estado = (prontidao.etapas || []).find((x) => x.id === 'periodo');
    ...
    const liberado = estado?.status === 'VERDE';
    el.innerHTML = `<header class="central-etapa-topo"><div><div class="olho">CENTRAL DE DADOS</div><h1>Período analisado</h1><p>Defina uma única janela de competências para importações, conferência de completude e relatórios desta empresa.</p></div><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><span class="prontidao-luz ${liberado ? 'liberado' : 'pendente'}"><span aria-hidden="true">●</span> ${liberado ? 'LIBERADO' : 'PENDÊNCIAS'}</span><button class="btn" id="editarPeriodoAnalisado">${periodo ? 'Alterar período' : 'Definir período'}</button></div></header>` + resumo;
    el.querySelector('#editarPeriodoAnalisado')?.addEventListener('click', () => A.modal({
      ...
      descricao: 'Defina o exercício da análise e a janela de apurações desta empresa. O sistema converte as competências em datas automaticamente.',
      corpo: `<div class="grade g2">${A.campo('competencia_inicio', 'Exercício — competência inicial', periodo?.competencia_inicio || dados.competencia_abertura || '', 'month', dados.competencia_abertura ? `min="${dados.competencia_abertura}"` : '')}${A.campo('competencia_fim', 'Exercício — competência final', periodo?.competencia_fim || '', 'month')}</div>${dados.data_abertura ? `<div class="aviso mini">Empresa aberta em <b>${A.esc(dados.data_abertura.split('-').reverse().join('/'))}</b>. O período pode começar somente a partir de <b>${A.esc(dados.competencia_abertura)}</b>.</div>` : ''}<div class="grade g2">${A.campo('apuracao_meses', 'Meses de apuração a buscar (mínimo 12)', periodo?.apuracao_meses || 12, 'number','min="12" step="1"')}${A.selecao('apuracao_inclui_exercicio','Como aplicar a janela',[{v:'1',t:'Considerar o exercício selecionado'},{v:'0',t:'Buscar antes do exercício selecionado'}],String(periodo?.apuracao_inclui_exercicio ?? 1))}</div><div class="aviso mini">Exemplo: 12 meses considerando o exercício termina na competência final; 12 meses fora do exercício termina no mês anterior à competência inicial. Esta ação não exclui dados existentes.</div>`,
...
Telas.executarMotor = async (el) => {
  const render = async () => {
    const [prontidao, motor, acompanhamento] = await Promise.all([A.api(`/empresas/${S.empresaId}/motor/prontidao`), A.api(`/empresas/${S.empresaId}/motor`), A.api(`/empresas/${S.empresaId}/motor/status`)]);
    const liberado=Boolean(prontidao.motor?.liberado); const pendencias=prontidao.motor?.pendencias || [];
    const job=acompanhamento.job || null;
    const nomesEstado={PENDENTE:'Na fila',CONCLUIDO:'Concluído',PROCESSANDO:'Em execução',AGUARDANDO:'Aguardando',FALHOU:'Falhou'};
    const classeEstado={CONCLUIDO:'c',PROCESSANDO:'a',AGUARDANDO:'',FALHOU:'a'};
    const etapas=(job?.etapas || []).map((etapa)=>`<div class="cartao" style="box-shadow:none;border:1px solid var(--linha);margin:0"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px"><b>${A.esc(etapa.titulo)}</b><span class="tag ${classeEstado[etapa.estado] || ''}">${A.esc(nomesEstado[etapa.estado] || etapa.estado)}</span></div><p class="mini" style="margin-bottom:0">${A.esc(etapa.detalhe)}</p></div>`).join('');
    const estadoJob=String(job?.estado || job?.status || '');
    const naFila=estadoJob==='PENDENTE';
    const ativo=job && ['PENDENTE','PROCESSANDO','AGUARDANDO','SINCRONIZANDO_CADASTRO','CALCULANDO','PUBLICANDO'].includes(estadoJob);
    el.innerHTML=cab('MOTOR TRIBUTÁRIO · PROCESSAMENTO','Executar motor','Processa somente quando os dados mínimos da operação atual estiverem resolvidos. O cálculo permanece bloqueado por empresa até a prontidão ficar verde.')+
      `<section class="cartao"><div class="cabecalho-lista"><div><div class="olho">PRONTIDÃO PARA CÁLCULO</div><h2><span style="color:${liberado?'#138a4b':'#c03532'}">●</span> ${liberado?'Motor pronto para cálculo':'Motor bloqueado'}</h2><p class="desc">${liberado?'Documentos fiscais e Outras receitas estão resolvidos no período analisado.':'Há pendências obrigatórias antes do processamento.'}</p></div><span class="tag ${liberado?'c':'a'}">${liberado?'LIBERADO':'PENDÊNCIAS'}</span></div>${liberado?`<div class="aviso bom">Período: ${A.esc(prontidao.periodo.competencia_inicio)} até ${A.esc(prontidao.periodo.competencia_fim)}.</div>`:`<div class="aviso alto"><ul>${pendencias.map((p)=>`<li>${A.esc(p)}</li>`).join('')}</ul><button class="btn pq" id="abrirPendenciasMotor">Resolver pendências</button></div>`}${motor.execucao?`<div class="aviso bom" style="margin-top:12px"><b>Última execução:</b> ${A.esc(motor.execucao.criado_em || '—')} · ${motor.execucao.itens || 0} item(ns) fiscal(is) processado(s).</div>`:''}<div style="margin-top:16px"><button class="btn ouro" id="executarMotorCentral" ${liberado?'':'disabled'}>Executar motor</button></div><div id="statusMotorCentral" style="margin-top:12px"></div></section>${job ? `<section class="cartao" style="margin-top:16px"><div class="cabecalho-lista"><div><div class="olho">ACOMPANHAMENTO DA EXECUÇÃO</div><h2>O que o motor fez</h2><p class="desc">Job ${A.esc(job.id)} · ${A.esc(nomesEstado[job.estado] || nomesEstado[job.status] || job.status)}</p></div><span class="tag ${naFila?'':ativo?'a':job.estado==='CONCLUIDO'?'c':'a'}">${naFila?'NA FILA':ativo?'EM PROCESSAMENTO':job.estado==='CONCLUIDO'?'CONCLUÍDO':'ATENÇÃO'}</span></div>${naFila?'<p class="mini" style="margin:8px 0 0">Aguardando um trabalhador assumir o job. Esta tela é atualizada automaticamente.</p>':''}<div class="grade g2" style="margin-top:14px">${etapas}</div>${job.erro ? `<div class="aviso alto" style="margin-top:12px"><b>Detalhe do erro:</b> ${A.esc(job.erro)}</div>` : ''}</section>` : ''}`;
    el.querySelector('#abrirPendenciasMotor')?.addEventListener('click',()=>{ S.aba.centralDados='documentos'; A.ir('dados'); });
    el.querySelector('#executarMotorCentral')?.addEventListener('click',async()=>{ const box=el.querySelector('#statusMotorCentral'); box.innerHTML='<div class="carregando">Conferindo os documentos fiscais e iniciando o processamento…</div>'; try { const r=await A.api(`/empresas/${S.empresaId}/motor/executar`,{metodo:'POST',corpo:{}}); const rec=r.reconciliacao_documental; box.innerHTML=`<div class="aviso bom"><b>Processamento iniciado.</b><br>${rec ? `${A.esc(rec.movimentos)} item(ns) fiscal(is) conferido(s) na fonte compartilhada. ` : ''}Job ${A.esc(r.processamento_id || r.id || '')}. A fotografia anterior permanece ativa até a publicação da nova.</div>`; } catch(e) { box.innerHTML=`<div class="aviso alto">${A.esc(e.message)}</div>`; } });
    if (ativo) setTimeout(() => { if (el.isConnected) render().catch(() => {}); }, 2500);
  };
  await render();
};
Telas.planejamento = async (el) => {
  ...
  const anosDaAnalise = (analise) => {
    const ano = Number(String(analise?.periodo_base_fim || analise?.periodo_base_inicio || '').slice(0, 4)) || anoAtual;
  ...
  const render = async () => {
    ...
    el.querySelectorAll('[data-comparador-detalhe]').forEach((b)=>b.onclick=()=>{
      const c=(comparativoRegimes?.cenarios||[]).find((x)=>x.chave===b.dataset.comparadorDetalhe);
      if (!c) return;
      const componentes=Object.entries(c.componentes_disponiveis||{}).map(([chave,valor])=>({
        chave, valor:Number(valor?.valor), natureza:valor?.natureza||'INDETERMINADO',
        origem:valor?.origem||valor?.regra||'Memória do comparador', detalhes:valor?.detalhes||null,
      }));
      A.modal({titulo:`Memória — ${c.rotulo}`,largura:980,
        descricao:'Componentes usados nesta comparação. A abertura é somente de leitura e não altera o regime cadastrado.',
        corpo:`${A.tabela([{t:'Componente',r:x=>A.esc(rotuloComponenteRegime(x.chave))},{t:'Valor',num:true,r:x=>Number.isFinite(x.valor)?A.moeda(x.valor):'INDETERMINADO'},{t:'Natureza',r:x=>`<span class="tag ${x.natureza==='REAL'?'c':x.natureza==='SIMULADO'?'b':'n'}">${A.esc(x.natureza)}</span>`},{t:'Fonte / regra',r:x=>A.esc(x.origem)}],componentes,{vazio:'Nenhum componente foi determinado para este regime.'})}${c.premissas_utilizadas?.length?`<div class="aviso info" style="margin-top:14px"><b>Premissas utilizadas</b><br>${c.premissas_utilizadas.map(A.esc).join('<br>')}</div>`:''}${c.pendencias?.length?`<div class="aviso atencao" style="margin-top:12px"><b>Pendências</b><br>${c.pendencias.map(A.esc).join('<br>')}</div>`:''}`});
    });
    ...
    el.querySelector('[data-importar-planejamento]')?.addEventListener('click', () => {
      const anos = anosDaAnalise(detalhe.analise);
      A.modal({ titulo:'Importar dados para o planejamento', largura:720,
        descricao:`A análise exige o período analisado (${anos[0]}) mais os dois anos anteriores (${anos[1]} e ${anos[2]}). Folha e pró-labore são tratados no módulo próprio.`,
        corpo:`<div class="aviso"><b>Cobertura esperada: ${anos.join(', ')}</b><br><span class="mini">Envie as 12 competências de cada ano aplicável. Empresa iniciada após ${anos[2]}: envie o histórico disponível; a projeção ficará identificada e exigirá justificativa.</span></div>${A.selecao('empresa_id','Empresa',detalhe.empresas.map(x=>({v:x.id,t:x.razao_social})),detalhe.empresas[0]?.id)}${A.selecao('tipo','Dados a importar',[{v:'pgdas',t:`PGDAS do Simples (${anos.join(', ')})`},{v:'receita',t:`Faturamento mensal (${anos.join(', ')})`},{v:'pis_cofins',t:`Apuração PIS/Cofins (${anos.join(', ')})`}],'receita')}<label class="campo"><span id="rotuloArquivoPlanejamento">Arquivo XLSX, XLS ou CSV</span><input type="file" id="arquivoPlanejamento" accept=".xlsx,.xls,.csv"></label><div id="ajudaArquivoPlanejamento" class="aviso mini">Faturamento mensal é importado por planilha estruturada.</div>`,
        confirmar:'Importar',
        aoAbrir:(modal)=>{ const tipo=modal.querySelector('[name="tipo"]'); const arquivo=modal.querySelector('#arquivoPlanejamento'); const rotulo=modal.querySelector('#rotuloArquivoPlanejamento'); const ajuda=modal.querySelector('#ajudaArquivoPlanejamento'); const atualizar=()=>{ const modo=tipo?.value; const pgdas=modo==='pgdas'; const pis=modo==='pis_cofins'; arquivo.accept=pgdas?'.xlsx,.xls,.csv,.pdf,.png,.jpg,.jpeg':pis?'.xlsx,.xls,.csv,.pdf,.txt':'.xlsx,.xls,.csv'; rotulo.textContent=pgdas?'PGDAS: planilha, PDF ou imagem':pis?'Apuração PIS/Cofins: PDF, XLSX, CSV ou relatório ERP':'Arquivo XLSX, XLS ou CSV'; ajuda.textContent=pgdas?'Planilhas PGDAS são estruturadas; PDF, PNG ou JPG são lidos pelo Azure e exigem confirmação antes de entrarem no histórico.':pis?'A apuração é lida e permanece em revisão até a confirmação dos campos extraídos.':'Faturamento mensal é importado por planilha estruturada.'; }; tipo?.addEventListener('change',atualizar); atualizar(); },
        aoConfirmar:async(f,modal)=>{ const arq=modal.querySelector('#arquivoPlanejamento')?.files?.[0]; if(!arq) throw new Error('Selecione o arquivo.'); const fd=new FormData(); fd.append('arquivo',arq); const extensao=(arq.name.split('.').pop()||'').toLowerCase(); const documentoPgdas=['pdf','png','jpg','jpeg'].includes(extensao); if(f.tipo==='pgdas'&&documentoPgdas){ await A.api(`/empresas/${f.empresa_id}/pgdas/ingestao`,{metodo:'POST',corpo:fd}); A.toast('PGDAS lido pelo Azure. Revise e confirme os campos antes de criar uma nova fotografia.','ok'); return; } if(f.tipo==='pis_cofins'){ fd.append('tipo_documento', extensao==='pdf'?'PDF':extensao==='csv'?'CSV':extensao==='xlsx'||extensao==='xls'?'XLSX':'RELATORIO_ERP'); await A.api(`/empresas/${f.empresa_id}/apuracoes-pis-cofins/ingestao`,{metodo:'POST',corpo:fd}); A.toast('Apuração PIS/Cofins lida. Revise e confirme os campos antes de criar uma nova fotografia.','ok'); return; } const rota=f.tipo==='pgdas'?`/empresas/${f.empresa_id}/importar/pgdas`:`/empresas/${f.empresa_id}/importar/receitas-sem-dfe`; await A.api(rota,{metodo:'POST',corpo:fd}); A.toast('Importação concluída. Crie uma nova fotografia para atualizar a análise.','ok'); }
      });
    });
    ...
    el.querySelector('[data-premissa-planejamento]')?.addEventListener('click', ()=>A.modal({titulo:'Premissas de margem e preço',descricao:'A premissa é auditável e não altera dados oficiais.',corpo:`${A.selecao('campo','Premissa',[{v:'estrategia_preco',t:'Estratégia de preço'},{v:'impacto_reforma_percentual',t:'Impacto da reforma sobre faturamento'},{v:'percentual_repasse',t:'Percentual de repasse'}],'estrategia_preco')}${A.campo('valor','Valor','MANTER_PRECO','text','placeholder="MANTER_PRECO, PRESERVAR_MARGEM, REPASSE_PARCIAL ou 0,03"')}<p class="mini">Impacto e repasse usam decimal: 0,03 = 3%; 0,50 = 50%.</p>${A.selecao('tipo','Tipo',[{v:'MARGEM',t:'Margem / preço'},{v:'FINANCEIRA',t:'Financeira'}],'MARGEM')}${A.area('justificativa','Justificativa','',2)}`,confirmar:'Registrar',aoConfirmar:async(f)=>{f.valor=String(f.valor).replace(',','.');await A.api(`/planejamento/analises/${atualId}/premissas`,{metodo:'POST',corpo:f}); A.toast('Premissa registrada na trilha de auditoria.', 'ok'); await render();}}));
    el.querySelector('[data-assistente-planejamento]')?.addEventListener('click', async()=>{
      const h=await A.api(`/planejamento/analises/${atualId}/assistente`);
      const historico=(h.interacoes||[]).slice(0,4).map(x=>`<article class="aviso" style="margin-top:10px"><b>Pergunta:</b> ${A.esc(x.pergunta)}<br><br><b>Resposta:</b><br>${A.esc(x.resposta).replaceAll('\n','<br>')}<br><span class="mini">Fotografia ${A.esc(x.snapshot_id)} · ${A.esc(x.criado_em||'')}</span></article>`).join('');
      A.modal({titulo:'Assistente Tributário',descricao:h.configurado?'IA de apoio: explica exclusivamente a fotografia e os cenários calculados. Não altera motor, regras, cadastro ou regime.':'A IA ainda não está configurada. Cadastre a chave Anthropic em Base de conhecimento → Configuração da IA.',largura:900,corpo:`${h.configurado?A.area('pergunta','Pergunte sobre a análise','Ex.: Quais premissas podem alterar a recomendação?',4):''}${historico?`<h3 style="margin-top:18px">Consultas anteriores</h3>${historico}`:''}`,confirmar:h.configurado?'Perguntar':'Fechar',aoConfirmar:h.configurado?async(f,modal)=>{const r=await A.api(`/planejamento/analises/${atualId}/assistente`,{metodo:'POST',corpo:{pergunta:f.pergunta}});modal.querySelector('.modal-corpo').insertAdjacentHTML('beforeend',`<article class="aviso ok" style="margin-top:14px"><b>Resposta do assistente</b><br><br>${A.esc(r.resposta).replaceAll('\n','<br>')}<br><span class="mini">Registrada na trilha desta fotografia.</span></article>`);}:undefined});
    });
  };
  await render();
};
function quadroComparativoRegimes(comparativo, selecionado) {
  ...
  const linhas=[['DAS / Simples', 'das_remanescente_pgdas'],['PIS / Cofins', 'pis_cofins'],['IRPJ / CSLL', 'irpj_csll'],['CPP / folha', 'inss_patronal'],['CBS', 'cbs'],['IBS', 'ibs_hibrida_motor']];
  const linhasVisiveis=linhas.filter(([,chave])=>cenarios.some((c)=>Number.isFinite(valor(c,chave))));
  const maior=Math.max(...cenarios.map((c)=>Number(c.tributos_estimados)||0),0);
  const cartao=(c)=>{
    const total=Number(c.tributos_estimados);
    const componentes=linhas.map(([rotulo,chave])=>({rotulo,chave,valor:valor(c,chave)})).filter((x)=>Number.isFinite(x.valor));
    return `<article class="cartao ${atual===c.chave?'selecionado':''}" style="margin:0;display:flex;flex-direction:column;gap:12px;border-top:3px solid ${atual===c.chave?'var(--ouro)':'#d9e4ea'}">
      <button type="button" data-comparador-regime="${A.esc(c.chave)}" style="all:unset;cursor:pointer;display:block"><b>${A.esc(c.rotulo)}</b><span class="mini bloco">${A.esc(c.natureza||'INDETERMINADO')} · ${c.status==='COMPLETO'?'comparável':`${(c.pendencias||[]).length} pendência(s)`}</span><strong style="display:block;font-size:22px;margin-top:9px">${Number.isFinite(total)?A.moeda(total):'INDETERMINADO'}</strong><span class="mini">Carga efetiva: ${c.carga_efetiva_percentual===null?'—':A.pct(c.carga_efetiva_percentual)}</span></button>
      <div style="display:grid;gap:8px">${componentes.length?componentes.map((x)=>{const percentual=maior?Math.min(100,(x.valor/maior)*100):0;return `<div><div style="display:flex;justify-content:space-between;gap:8px;font-size:12px"><span>${A.esc(x.rotulo)}</span><b class="mono">${A.moeda(x.valor)}</b></div><div style="height:5px;border-radius:99px;background:#e7edf1;margin-top:4px;overflow:hidden"><i style="display:block;height:100%;width:${percentual}%;background:${x.chave==='cbs'||x.chave==='ibs_hibrida_motor'?'#ef4b13':'var(--azul)'}"></i></div></div>`;}).join(''):'<span class="mini">Componentes ainda não determinados.</span>'}</div>
      <button type="button" class="btn pq ${atual===c.chave?'ouro':'vazio'}" data-comparador-detalhe="${A.esc(c.chave)}">Ver detalhes</button>
    </article>`;
  };
  return `<section class="planejamento-comparativo-regimes"><div class="planejamento-secao"><div><div class="olho">COMPARAÇÃO DE REGIMES</div><h3>Impacto tributário por cenário</h3><p class="mini">Mesma base econômica: ${A.moeda(comparativo.receita_analisada)}. Valores reais, calculados e simulados permanecem identificados.</p></div><span class="tag ${comparativo.status_comparacao==='COMPLETA'?'c':'b'}">${A.esc(comparativo.status_comparacao)}</span></div><div class="grade g4" style="margin:12px 0">${cenarios.map(cartao).join('')}</div><div class="tabela-wrap">${A.tabela([{t:'Componente',r:x=>A.esc(x[0])},...cenarios.map((c)=>({t:A.esc(c.rotulo),num:true,r:x=>{const v=valor(c,x[1]);return Number.isFinite(v)?A.moeda(v):'—';}}))],linhasVisiveis.map((x)=>[...x]),{vazio:'Ainda não há componentes tributários comparáveis.'})}</div>${comparativo.pendencias?.length?`<div class="aviso atencao" style="margin-top:12px"><b>O que falta para comparar todos os regimes:</b> ${A.esc(comparativo.pendencias[0])}</div>`:''}</section>`;
}
function rotuloComponenteRegime(chave) { return ({ das_remanescente_pgdas:'DAS / Simples',pis_cofins:'PIS / Cofins',irpj_csll:'IRPJ / CSLL',inss_patronal:'CPP / folha',cbs_motor_existente:'CBS',cbs_hibrida_motor:'CBS',ibs_hibrida_motor:'IBS' })[chave] || String(chave||'—').replaceAll('_',' '); }
function detalhePlanejamento(d, resultados, recomendado) {
  ...
  const coletas=snap?.dados?.necessidades_coleta||[];
  const necessidades=coletas.flatMap(x=>x.itens.filter(i=>!i.recebido).map(i=>`${x.empresa}: ${i.titulo}`));
  const alertas=coletas.map(x=>x.alerta).filter(Boolean);
  const status=String(a.status||'').replaceAll('_',' ');
  return `<div class="planejamento-resumo"><div><span class="olho">Empresas da análise</span><strong>${empresas.map((x)=>A.esc(x.razao_social)).join(' · ')}</strong></div><div><span class="olho">Fotografia</span><strong>${snap?`Versão ${snap.versao}`:'Pendente'}</strong><small>${snap?'dados congelados':'crie a primeira fotografia'}</small></div><div><span class="olho">Decisão</span><strong>${A.esc(status)}</strong><small>não altera o cadastro</small></div><div><span class="olho">Recomendação</span><strong>${recomendado?A.esc(recomendado.cenario.replaceAll('_',' ')):'Validação necessária'}</strong><small>${recomendado?'cenário completo':'aguarda dados suficientes'}</small></div></div><div class="planejamento-acoes"><button class="btn" data-executar-planejamento>Executar cenários</button><button class="btn vazio" data-importar-planejamento>Importar histórico</button><button class="btn vazio" data-fotografia-planejamento>Atualizar fotografia</button><button class="btn vazio" data-cnaes-planejamento>Consultar CNAEs</button><button class="btn vazio" data-premissa-planejamento>Premissas</button>${linhas.length?'<button class="btn vazio" data-assistente-planejamento>Assistente tributário</button>':''}${a.status==='EM_REVISAO'?'<button class="btn ouro" data-aprovar-planejamento>Aprovar decisão</button>':''}</div>${necessidades.length?`<div class="aviso atencao planejamento-coleta"><b>Histórico necessário para qualificar a recomendação</b><span>${necessidades.map(A.esc).join(' · ')}</span>${alertas.map(x=>`<p class="acao">${A.esc(x)}</p>`).join('')}</div>`:''}${linhas.length?`<section class="planejamento-cenarios"><div class="planejamento-secao"><div><h3>Cenários comparados</h3><p class="mini">Valores calculados sobre a fotografia vigente.</p></div></div>${A.tabela([{t:'Cenário',r:x=>A.esc(x.cenario.replaceAll('_',' '))},{t:'Tributos',num:true,r:x=>x.tributos===null?'INDICATIVO':A.moeda(x.tributos)},{t:'Carga efetiva',num:true,r:x=>x.carga===null?'—':A.pct(x.carga)},{t:'Δ baseline',num:true,r:x=>x.economia===null?'—':A.moeda(x.economia)},{t:'Confiança',r:x=>`<span class="tag ${x.confianca==='ALTA'?'c':x.confianca==='MEDIA'?'b':'n'}">${A.esc(x.confianca)}</span>`},{t:'Pendências',r:x=>String(x.pendencias)}],linhas)}</section>`:'<div class="aviso planejamento-vazio"><b>Próximo passo</b>Importe o histórico disponível e execute os cenários para comparar os regimes.</div>'}<footer class="planejamento-rodape"><b>Trilha de governança</b> · ${(d.premissas||[]).length} premissa(s) · ${(d.eventos||[]).length} evento(s) auditável(is). Dados reais, projetados e manuais permanecem identificados.</footer>`;
}
})();
