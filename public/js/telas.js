...
Telas.executarMotor = async (el) => {
  const render = async () => {
    ...
    el.querySelector('#executarMotorCentral')?.addEventListener('click',async()=>{ const box=el.querySelector('#statusMotorCentral'); box.innerHTML='<div class="carregando">Registrando o processamento seguro…</div>'; try { const r=await A.api(`/empresas/${S.empresaId}/motor/executar`,{metodo:'POST',corpo:{}}); box.innerHTML=`<div class="aviso bom"><b>Processamento registrado.</b><br>Job ${A.esc(r.processamento_id || r.id || '')}. O worker conferirá a fonte fiscal antes de calcular. A fotografia anterior permanece ativa até a publicação da nova.</div>`; } catch(e) { box.innerHTML=`<div class="aviso alto">${A.esc(e.message)}</div>`; } });
