...
Telas.periodoAnalisado = async (el) => {
  ...
  const render = async () => {
    ...
    el.querySelector('#editarPeriodoAnalisado')?.addEventListener('click', () => A.modal({
      ...
      corpo: `<div class="grade g2">${A.campo('competencia_inicio', 'Exercício — competência inicial', periodo?.competencia_inicio || '', 'month')}${A.campo('competencia_fim', 'Exercício — competência final', periodo?.competencia_fim || '', 'month')}</div><div class="grade g2">${A.campo('apuracao_meses', 'Meses de apuração a buscar (mínimo 12)', periodo?.apuracao_meses || 12, 'number','min="12" step="1"')}${A.selecao('apuracao_inclui_exercicio','Como aplicar a janela',[{v:'1',t:'Considerar o exercício selecionado'},{v:'0',t:'Buscar antes do exercício selecionado'}],String(periodo?.apuracao_inclui_exercicio ?? 1))}</div><div class="aviso mini">Exemplo: 12 meses considerando o exercício termina na competência final; 12 meses fora do exercício termina no mês anterior à competência inicial. Esta ação não exclui dados existentes.</div>`,
