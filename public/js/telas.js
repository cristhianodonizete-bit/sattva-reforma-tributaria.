...
Telas.planejamento = async (el) => {
  ...
  const anoAtual = new Date().getFullYear(), anosHistorico = [anoAtual - 1, anoAtual - 2];
  const render = async () => {
    ...
    el.innerHTML = cab('PLANEJAMENTO TRIBUTÁRIO', 'Compare regimes com dados históricos, projeções e decisão auditável.', 'O estudo não altera documentos, regime cadastrado, apuração ou motor operacional.', '<button class="btn" id="novaAnalisePlanejamento">Nova análise</button>') +
    ...
    el.querySelector('#novaAnalisePlanejamento').onclick = () => {
      ...
      A.modal({ titulo:'Nova análise tributária', descricao:'A fotografia dos dados será congelada. Premissas não sobrescrevem dados reais.', largura:760,
        corpo:`${A.campo('titulo','Título','Planejamento tributário')}${A.area('descricao','Objetivo / observação','',2)}<div class="aviso"><b>Dados solicitados neste módulo</b><br>Simples Nacional: PGDAS de ${anosHistorico[0]} e ${anosHistorico[1]}. Lucro Presumido ou Real: faturamento mensal de ${anosHistorico[0]} e ${anosHistorico[1]}. Folha e pró-labore permanecem no módulo próprio.<br><span class="mini">Quando a empresa tiver iniciado depois de ${anosHistorico[1]}, a projeção será usada e a justificativa ficará registrada na análise.</span></div><label class="campo"><span>Empresas da análise</span><div class="check-list">${empresas.map((x,i)=>`<label><input type="checkbox" name="empresa_${x.id}" ${i===0?'checked':''}> ${A.esc(x.razao_social)}</label>`).join('')}</div></label>`,
    ...
    el.querySelector('[data-importar-planejamento]')?.addEventListener('click', ()=>A.modal({titulo:'Importar dados para o planejamento',descricao:`Envie somente os dois anos anteriores ao ano da análise: ${anosHistorico[0]} e ${anosHistorico[1]}. Folha e pró-labore são tratados no módulo próprio.`,largura:720,corpo:`${A.selecao('empresa_id','Empresa',detalhe.empresas.map(x=>({v:x.id,t:x.razao_social})),detalhe.empresas[0]?.id)}${A.selecao('tipo','Dados a importar',[{v:'pgdas',t:`PGDAS do Simples (${anosHistorico[0]} e ${anosHistorico[1]})`},{v:'receita',t:`Faturamento mensal (${anosHistorico[0]} e ${anosHistorico[1]})`}],'receita')}<label class="campo"><span id="rotuloArquivoPlanejamento">Arquivo XLSX, XLS ou CSV</span><input type="file" id="arquivoPlanejamento" accept=".xlsx,.xls,.csv"></label><div id="ajudaArquivoPlanejamento" class="aviso mini">Faturamento mensal é importado por planilha estruturada.</div><p class="mini">Se a empresa iniciou após ${anosHistorico[1]}, envie o período disponível. A análise usará projeção e exigirá justificativa.</p>`,confirmar:'Importar',aoAbrir:(modal)=>{const tipo=modal.querySelector('[name="tipo"]');const arquivo=modal.querySelector('#arquivoPlanejamento');const rotulo=modal.querySelector('#rotuloArquivoPlanejamento');const ajuda=modal.querySelector('#ajudaArquivoPlanejamento');const atualizar=()=>{const pgdas=tipo?.value==='pgdas';arquivo.accept=pgdas?'.xlsx,.xls,.csv,.pdf,.png,.jpg,.jpeg':'.xlsx,.xls,.csv';rotulo.textContent=pgdas?'PGDAS: planilha, PDF ou imagem':'Arquivo XLSX, XLS ou CSV';ajuda.textContent=pgdas?'Reutiliza a importação oficial de PGDAS: planilhas são estruturadas; PDF, PNG ou JPG são lidos pelo Azure e exigem confirmação antes de entrarem no histórico.':'Faturamento mensal é importado por planilha estruturada.';};tipo?.addEventListener('change',atualizar);atualizar();},aoConfirmar:async(f,modal)=>{const arq=modal.querySelector('#arquivoPlanejamento')?.files?.[0];if(!arq)throw new Error('Selecione o arquivo.');const fd=new FormData();fd.append('arquivo',arq);const extensao=(arq.name.split('.').pop()||'').toLowerCase();const documentoPgdas=['pdf','png','jpg','jpeg'].includes(extensao);if(f.tipo==='pgdas'&&documentoPgdas){await A.api(`/empresas/${f.empresa_id}/pgdas/ingestao`,{metodo:'POST',corpo:fd});A.toast('PGDAS lido pelo Azure. Revise e confirme os campos no módulo Dados antes de criar uma nova fotografia.','ok');return;}const rota=f.tipo==='pgdas'?`/empresas/${f.empresa_id}/importar/pgdas`:`/empresas/${f.empresa_id}/importar/receitas-sem-dfe`;await A.api(rota,{metodo:'POST',corpo:fd});A.toast('Importação concluída. Crie uma nova fotografia para atualizar a análise.','ok');}}));
