...
Telas.empresas = async (el) => {
  ...
  el.querySelectorAll('[data-qsa]').forEach((b) => { b.onclick = async () => {
    ...
    const abrir = async () => {
      ...
      A.modal({ titulo:`Quadro societário — ${e.razao_social}`, largura:900, confirmar:'Fechar', corpo:`<div class="aviso"><b>ATENDE_200044: ${A.esc(d.atende_200044)}</b><br>${A.esc(d.motivo)}</div><p class="mini">A condição exige sócio brasileiro com participação igual ou superior a 20%. Percentual ausente nunca é presumido.</p><button class="btn" id="enriquecerQsa">Consultar cadastro</button><button class="btn vazio" id="adicionarQsa">Adicionar sócio</button><button class="btn vazio" id="sanearAnexoXi">Enriquecer contrapartes e reprocessar Anexo XI</button><table class="tabela" style="margin-top:12px"><thead><tr><th>Sócio</th><th>Qualificação</th><th>Participação</th><th>Brasileiro</th><th></th></tr></thead><tbody>${linhas || '<tr><td colspan="5">Nenhum sócio disponível.</td></tr>'}</tbody></table>`, aoConfirmar: async()=>{} });
