...
const estadoPrecificacaoVersionada = { modalidade:'', status:'', selecionados:new Set() };
async function telaPrecificacaoVersionada(el) {
  const [d,premissasDados,conferencia]=await Promise.all([A.api(`/empresas/${S.empresaId}/precificacao/itens`),A.api(`/empresas/${S.empresaId}/precificacao/premissas-comerciais`),A.api(`/empresas/${S.empresaId}/precificacao/conferencia`)]); const cenariosDados=await A.api(`/empresas/${S.empresaId}/cenarios/lista`); const todos=d.itens || []; const premissas=premissasDados.premissas||[]; const cenariosOperacionais=cenariosDados.cenarios||[]; const ep=estadoPrecificacaoVersionada;
  const itens=todos.filter(x=>(!ep.modalidade||x.modalidade===ep.modalidade)&&(!ep.status||x.calculo_status===ep.status));
  el.innerHTML=cab('Módulo 2','Precificação versionada','Importe a base operacional da empresa; ela contém aquisição, locação ou composição, sem separar custo e despesa. “Novo item” é destinado a exceções.',`<button class="btn" id="importarBaseOperacionalPrec">Importar base operacional</button><button class="btn vazio" id="baixarBaseOperacionalPrec">Modelo base operacional</button><button class="btn vazio" id="premissasComerciaisPrec">Premissas comerciais</button><button class="btn" id="calcularBaseLotePrec">Calcular base em lote</button><button class="btn vazio" id="importarPlanilhaPrec">Importar itens</button><button class="btn vazio" id="baixarModeloPrecVersionada">Modelo de itens XLSX</button><button class="btn vazio" id="novoItemPrec">Novo item</button><button class="btn vazio" id="creditosGlobaisPrec">Créditos globais</button><button class="btn vazio" id="importacoesPrec">Lotes de importação</button><button class="btn vazio" id="validarLotePrec">Validar calculados</button><button class="btn vazio" id="aprovarLotePrec">Aprovar validados</button><button class="btn vazio" id="reprocessarTodosPrec">Reprocessar calculados</button><button class="btn vazio" id="voltarPrecVersionada">Voltar</button>`) +
    `<div class="grade g3">${A.kpi('Itens ativos',todos.filter(x=>x.ativo).length)}${A.kpi('Versões calculadas',todos.filter(x=>x.calculo_id).length)}${A.kpi('Vigentes',todos.filter(x=>x.calculo_status==='VIGENT').length)}</div><div class="cartao" style="margin-top:16px"><div class="linha"><div><h2>Conferência automática</h2><p class="mini">A conferência verifica vínculo com a base, classificação fiscal, memória de cálculo e evidência do motor. Ela não aprova versões automaticamente.</p></div><button class="btn vazio" id="abrirConferenciaPrec">Ver conferência</button></div><div class="grade g3">${A.kpi('Prontos para validar',conferencia.resumo.PRONTO_PARA_VALIDAR||0)}${A.kpi('Bloqueados',conferencia.resumo.BLOQUEADO||0)}${A.kpi('Pendentes de cálculo',conferencia.resumo.PENDENTE_DE_CALCULO||0)}</div></div><div class="cartao"><div class="filtros"><label>Modalidade<select id="filtroModalidadePrec"><option value="">Todas</option>${['REVENDA','LOCACAO','PRODUCAO_COMPOSICAO','MISTO_CONTRATO'].map(x=>`<option value="${x}" ${ep.modalidade===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Status<select id="filtroStatusPrec"><option value="">Todos</option>${['DRAFT','CALCULATED','VALIDATION','APPROVED','VIGENT','SUPERSEDED'].map(x=>`<option value="${x}" ${ep.status===x?'selected':''}>${x}</option>`).join('')}</select></label></div><h2>Itens e cálculos</h2>${A.tabela([
      {t:'',r:x=>`<input type="checkbox" data-selec-prec="${x.id}" ${ep.selecionados.has(x.id)?'checked':''}>`},
      {t:'Item',r:x=>`<b>${A.esc(x.codigo)}</b> · ${A.esc(x.descricao)}<div class="mini">${A.esc(x.modalidade)}${x.base_operacional_id?` · base ${A.esc(x.base_operacional_codigo||'vinculada')} · ${A.esc(x.base_classificacao_venda||'')}`:''}</div>`},
      ...
      {t:'Ações',r:x=>`<button class="btn pq vazio" data-calcular-prec="${x.id}">Calcular</button>${x.calculo_id?` <button class="btn pq vazio" data-historico-prec="${x.id}">Histórico</button> <button class="btn pq vazio" data-cenarios-prec="${x.id}">Cenários</button>${x.calculo_status==='CALCULATED'?` <button class="btn pq" data-status-prec="${x.calculo_id}" data-alvo="VALIDATION">Validar</button>`:''}${x.calculo_status==='VALIDATION'?` <button class="btn pq" data-status-prec="${x.calculo_id}" data-alvo="APPROVED">Aprovar</button>`:''}${x.calculo_status==='APPROVED'?` <button class="btn pq" data-status-prec="${x.calculo_id}" data-alvo="VIGENT">Vigorar</button>`:''}`:''}`},
    ],itens,{vazio:'Nenhum item versionado.'})}</div>`;
  document.getElementById('voltarPrecVersionada').onclick=()=>Telas.precificacao(el);
  document.getElementById('abrirConferenciaPrec').onclick=()=>A.modal({titulo:'Conferência automática do lote',largura:1100,corpo:`<p class="mini">Ação humana ainda é necessária para validar, aprovar e vigorar os resultados.</p>${A.tabela([{t:'Item',r:x=>`<b>${A.esc(x.codigo)}</b> · ${A.esc(x.descricao)}`},{t:'Situação',r:x=>A.esc(x.situacao)},{t:'Motivo',r:x=>A.esc(x.motivo)},{t:'Preço final',num:true,r:x=>x.resultado?.preco_final==null?'—':A.moeda(x.resultado.preco_final)},{t:'Versão',num:true,r:x=>x.versao||'—'}],conferencia.itens,{vazio:'Nenhum item para conferir.'})}`});
  const baixar=(url,nome)=>async()=>{try{const r=await fetch(url,{headers:{Authorization:`Bearer ${localStorage.getItem('sattva_token')||''}`}});if(!r.ok)throw new Error('Não foi possível gerar o modelo XLSX.');const u=URL.createObjectURL(await r.blob());const a=document.createElement('a');a.href=u;a.download=nome;a.click();URL.revokeObjectURL(u);}catch(e){A.toast(e.message,'erro');}};
  document.getElementById('baixarBaseOperacionalPrec').onclick=baixar(`/api/empresas/${S.empresaId}/precificacao/base-operacional/template`,'base-operacional-precificacao.xlsx');
  document.getElementById('importarBaseOperacionalPrec').onclick=()=>A.modal({titulo:'Importar base operacional',descricao:'Use este arquivo para locação, revenda/serviço e produção/composição. O regime do fornecedor é buscado no cadastro compartilhado e na base de 2024; sem registro, será considerado regular. NCM ou LC 116 + NBS são obrigatórios e o tratamento fiscal será classificado automaticamente.',corpo:'<input type="file" id="arquivoBaseOperacionalPrec" accept=".xlsx" required>',aoConfirmar:async()=>{const arq=document.getElementById('arquivoBaseOperacionalPrec').files[0];if(!arq)throw new Error('Selecione o arquivo XLSX.');const fd=new FormData();fd.append('arquivo',arq);const r=await A.api(`/empresas/${S.empresaId}/precificacao/base-operacional/importar`,{metodo:'POST',corpo:fd,formData:true});if(!r.importado)throw new Error((r.erros||[]).map(x=>`${x.aba} ${x.linha}: ${x.erro}`).join('\n'));const c=r.classificacoes||{};A.toast(`Base importada: ${r.bases} item(ns), ${r.componentes} componente(s). Integral: ${c.TRIBUTACAO_INTEGRAL||0}; redução: ${c.REDUCAO||0}; isenção: ${c.ISENCAO||0}; pendentes: ${c.PENDENTE_DE_ENQUADRAMENTO||0}.`,'ok');telaPrecificacaoVersionada(el);}});
  document.getElementById('premissasComerciaisPrec').onclick=()=>{const abrir=()=>A.modal({titulo:'Premissas comerciais',largura:900,corpo:`<p class="mini">As premissas ficam na empresa e podem ser reutilizadas em simulações, sem alterar a base operacional nem versões já calculadas. A Precificação usa o cenário operacional escolhido; Planejamento Tributário não é fonte direta de preço.</p>${A.tabela([{t:'Premissa',r:x=>A.esc(x.nome)},{t:'Cenário operacional',r:x=>A.esc(x.cenario_operacional_nome||'não definido')},{t:'Margem de contribuição',num:true,r:x=>A.pct(x.margem_contribuicao)},{t:'Percentuais por dentro',num:true,r:x=>A.pct(x.percentuais_por_dentro)},{t:'Padrão',r:x=>x.padrao?'Sim':'—'}],premissas,{vazio:'Nenhuma premissa cadastrada.'})}<button class="btn" id="novaPremissaComercial">Nova premissa</button>`});const modal=abrir();modal.fundo.querySelector('#novaPremissaComercial').onclick=()=>A.modal({titulo:'Nova premissa comercial',corpo:A.campo('nome','Nome do cenário','')+A.selecao('cenario_operacional_id','Cenário operacional',cenariosOperacionais.map(x=>({v:String(x.id),t:`${x.nome} · ${x.ano}`})),String(cenariosOperacionais.find(x=>x.tipo==='base')?.id||cenariosOperacionais[0]?.id||''))+A.campo('margem_contribuicao','Margem de contribuição (ex.: 0,20)','0','number')+A.campo('percentuais_por_dentro','Percentuais por dentro (ex.: 0,10)','0','number')+A.selecao('padrao','Usar como padrão',[{v:'false',t:'Não'},{v:'true',t:'Sim'}]),aoConfirmar:async dados=>{await A.api(`/empresas/${S.empresaId}/precificacao/premissas-comerciais`,{metodo:'POST',corpo:{...dados,padrao:dados.padrao==='true'}});telaPrecificacaoVersionada(el);}});};
  document.getElementById('calcularBaseLotePrec').onclick=()=>{const opcoes=premissas.map(x=>({v:String(x.id),t:`${x.nome}${x.padrao?' (padrão)':''}`}));A.modal({titulo:'Calcular base em lote',descricao:'Será criada uma nova versão para cada item com base operacional. Itens com enquadramento fiscal pendente serão bloqueados e não terão preço calculado.',corpo:A.selecao('premissa_id','Premissa comercial',opcoes,String(premissas.find(x=>x.padrao)?.id||premissas[0]?.id||''))+A.campo('ano','Ano da projeção','2027','number')+A.selecao('aplicar_creditos_globais','Aplicar créditos globais rateados',[{v:'false',t:'Não'},{v:'true',t:'Sim'}]),aoConfirmar:async dados=>{const ids=[...ep.selecionados];const r=await A.api(`/empresas/${S.empresaId}/precificacao/calcular-base-lote`,{metodo:'POST',corpo:{...dados,item_ids:ids,aplicar_creditos_globais:dados.aplicar_creditos_globais==='true'}});const bloqueados=r.bloqueados||[];A.toast(`${r.calculados} item(ns) calculado(s)${bloqueados.length?`; ${bloqueados.length} bloqueado(s) por pendência fiscal.`:'.'}`,bloqueados.length?'aviso':'ok');ep.selecionados.clear();telaPrecificacaoVersionada(el);}});};
  document.getElementById('baixarModeloPrecVersionada').onclick=async()=>{try{const r=await fetch(`/api/empresas/${S.empresaId}/precificacao-independente/template`,{headers:{Authorization:`Bearer ${localStorage.getItem('sattva_token')||''}`}});if(!r.ok)throw new Error('Não foi possível gerar o modelo XLSX.');const url=URL.createObjectURL(await r.blob());const a=document.createElement('a');a.href=url;a.download='modelo-precificacao-margem.xlsx';a.click();URL.revokeObjectURL(url);}catch(e){A.toast(e.message,'erro');}};
  document.getElementById('importarPlanilhaPrec').onclick=()=>A.modal({titulo:'Importar planilha de Precificação',descricao:'A planilha é validada antes de alterar a base. Itens fora do arquivo não são apagados.',corpo:'<input type="file" id="arquivoPrecVersionada" accept=".xlsx" required>',aoConfirmar:async()=>{const arq=document.getElementById('arquivoPrecVersionada').files[0];if(!arq)throw new Error('Selecione o arquivo XLSX.');const fd=new FormData();fd.append('arquivo',arq);const r=await A.api(`/empresas/${S.empresaId}/precificacao-independente/importar`,{metodo:'POST',corpo:fd,formData:true});if(!r.importado)throw new Error((r.erros||[]).map(x=>`${x.aba} ${x.linha}: ${x.erro}`).join('\n'));A.toast(`Planilha importada: ${r.produtos+r.servicos} item(ns).`,'ok');telaPrecificacaoVersionada(el);}});
  document.getElementById('filtroModalidadePrec').onchange=e=>{ep.modalidade=e.target.value;telaPrecificacaoVersionada(el);}; document.getElementById('filtroStatusPrec').onchange=e=>{ep.status=e.target.value;telaPrecificacaoVersionada(el);};
  el.querySelectorAll('[data-selec-prec]').forEach(b=>b.onchange=()=>{const id=Number(b.dataset.selecPrec);if(b.checked)ep.selecionados.add(id);else ep.selecionados.delete(id);});
  document.getElementById('reprocessarTodosPrec').onclick=async()=>{const ids=itens.filter(x=>x.calculo_id&&(!ep.selecionados.size||ep.selecionados.has(x.id))).map(x=>x.id);if(!ids.length)return A.toast('Não há versões calculadas para reprocessar.','erro');try{const r=await A.api(`/empresas/${S.empresaId}/precificacao/reprocessar`,{metodo:'POST',corpo:{item_ids:ids}});A.toast(`${r.resultado.filter(x=>x.status==='REPROCESSADO').length} item(ns) reprocessado(s).`,'ok');ep.selecionados.clear();telaPrecificacaoVersionada(el);}catch(e){A.toast(e.message,'erro');}};
  const statusLote=async(status)=>{const ids=itens.filter(x=>x.calculo_status===(status==='VALIDATION'?'CALCULATED':'VALIDATION')&&(!ep.selecionados.size||ep.selecionados.has(x.id))).map(x=>x.calculo_id);if(!ids.length)return A.toast('Não há versões elegíveis para esta ação.','erro');const r=await A.api(`/empresas/${S.empresaId}/precificacao/status-lote`,{metodo:'POST',corpo:{status,calculo_ids:ids}});A.toast(`${r.alterados} versão(ões) atualizada(s).`,'ok');ep.selecionados.clear();telaPrecificacaoVersionada(el);};
  document.getElementById('validarLotePrec').onclick=()=>statusLote('VALIDATION'); document.getElementById('aprovarLotePrec').onclick=()=>statusLote('APPROVED');
  document.getElementById('importacoesPrec').onclick=async()=>{const d=await A.api(`/empresas/${S.empresaId}/precificacao/importacoes`);const modal=A.modal({titulo:'Lotes de importação',largura:1000,corpo:A.tabela([{t:'Arquivo',r:x=>A.esc(x.arquivo)},{t:'Situação',r:x=>A.esc(x.status)},{t:'Linhas',num:true,r:x=>x.linhas},{t:'Erros',num:true,r:x=>x.erros||0},{t:'',r:x=>`<button class="btn pq vazio" data-lote-prec="${x.id}">Ver linhas</button>`}],d.lotes,{vazio:'Nenhum lote de importação.'})});modal.fundo.querySelectorAll('[data-lote-prec]').forEach(b=>b.onclick=async()=>{const r=await A.api(`/precificacao/importacoes/${b.dataset.lotePrec}/linhas`);A.modal({titulo:'Linhas do lote',largura:1100,corpo:A.tabela([{t:'Aba',r:x=>A.esc(x.aba)},{t:'Linha',num:true,r:x=>x.linha},{t:'Código',r:x=>A.esc(x.codigo_item)},{t:'Situação',r:x=>A.esc(x.status)},{t:'Erro',r:x=>A.esc(x.erro||'—')}],r.linhas)});});};
  document.getElementById('creditosGlobaisPrec').onclick=async()=>{const d=await A.api(`/empresas/${S.empresaId}/precificacao/creditos-globais`);const modal=A.modal({titulo:'Créditos globais',largura:900,corpo:`<p class="mini">Créditos globais só reduzem custo quando o critério de rateio estiver explícito. Créditos reais exigem evidência.</p>${A.tabela([{t:'Descrição',r:x=>A.esc(x.descricao)},{t:'Natureza',r:x=>A.esc(x.natureza)},{t:'Valor',num:true,r:x=>A.moeda(x.valor)},{t:'Rateio',r:x=>A.esc(x.criterio_rateio||'não definido')},{t:'Vigência',r:x=>A.esc(x.vigencia_inicio||'—')}],d.creditos,{vazio:'Nenhum crédito global cadastrado.'})}<button class="btn" id="novoCreditoGlobal">Novo crédito global</button>`});modal.fundo.querySelector('#novoCreditoGlobal').onclick=()=>A.modal({titulo:'Novo crédito global',corpo:A.campo('descricao','Descrição','')+A.selecao('natureza','Natureza',[{v:'GERENCIAL',t:'Gerencial'},{v:'REAL',t:'Real'},{v:'SIMULADO',t:'Simulado'}])+A.campo('valor','Valor','0','number')+A.campo('criterio_rateio','Critério de rateio','faturamento')+A.campo('percentual_rateio','Percentual de rateio','0','number')+A.area('evidencia','Evidência do crédito real',''),aoConfirmar:async b=>{await A.api(`/empresas/${S.empresaId}/precificacao/creditos-globais`,{metodo:'POST',corpo:b});modal.fechar();document.getElementById('creditosGlobaisPrec').click();}});};
  ...
  el.querySelectorAll('[data-calcular-prec]').forEach(b=>b.onclick=()=>{const item=todos.find(x=>x.id===Number(b.dataset.calcularPrec));const usaBase=Boolean(item?.base_operacional_id);const opcoesPremissas=premissas.map(x=>({v:String(x.id),t:`${x.nome}${x.padrao?' (padrão)':''}`}));const camposBase=A.campo('ano','Ano da projeção','2027','number')+A.selecao('premissa_id','Premissa comercial',opcoesPremissas,String(premissas.find(x=>x.padrao)?.id||premissas[0]?.id||''))+A.selecao('aplicar_creditos_globais','Aplicar créditos globais rateados',[{v:'false',t:'Não'},{v:'true',t:'Sim'}]);const camposManuais=A.campo('custo_bruto','Custo bruto','0','number')+A.campo('credito_direto','Crédito direto','0','number')+A.campo('credito_global_rateado','Crédito global rateado manual (se não aplicar automático)','0','number')+A.campo('aliquota_efetiva_cbs','Alíquota efetiva CBS (ex.: 0,09)','0','number')+A.selecao('tratamento','Tratamento',[{v:'INTEGRAL',t:'Integral'},{v:'REDUCAO_60',t:'Redução de 60%'},{v:'REDUCAO_30',t:'Redução de 30%'},{v:'ALIQUOTA_ZERO',t:'Alíquota zero'},{v:'ISENCAO',t:'Isenção'}])+A.area('evidencia_fiscal','Evidência do motor fiscal','');A.modal({titulo:'Calcular versão',descricao:usaBase?'O custo é formado automaticamente pela base operacional vinculada e a CBS vem do motor tributário conforme a classificação do item.':'Informe a evidência do motor fiscal para a alíquota efetiva de CBS.',corpo:camposBase+(usaBase?'<p class="mini">A composição, aquisição ou depreciação não é preenchida aqui: será usada diretamente da base da empresa.</p>':camposManuais),aoConfirmar:async dados=>{const t={tratamento:dados.tratamento,aliquota_efetiva_cbs:Number(dados.aliquota_efetiva_cbs)||0,preserva_credito:dados.tratamento==='ALIQUOTA_ZERO',estorna_credito:dados.tratamento==='ISENCAO'};await A.api(`/precificacao/itens/${b.dataset.calcularPrec}/calcular`,{metodo:'POST',corpo:{...dados,aplicar_creditos_globais:dados.aplicar_creditos_globais==='true',tratamentos:usaBase?[]:[t]}});telaPrecificacaoVersionada(el);}});});
  el.querySelectorAll('[data-status-prec]').forEach(b=>b.onclick=async()=>{try{await A.api(`/precificacao/calculos/${b.dataset.statusPrec}/status`,{metodo:'POST',corpo:{status:b.dataset.alvo}});A.toast(`Versão ${b.dataset.alvo==='VIGENT'?'tornada vigente':b.dataset.alvo.toLowerCase()+'.'}`,'ok');telaPrecificacaoVersionada(el);}catch(e){A.toast(e.message,'erro');}});
  el.querySelectorAll('[data-historico-prec]').forEach(b=>b.onclick=async()=>{const r=await A.api(`/precificacao/itens/${b.dataset.historicoPrec}/historico`);A.modal({titulo:`Histórico — ${r.item.descricao}`,largura:1000,corpo:A.tabela([{t:'Versão',num:true,r:x=>x.versao},{t:'Status',r:x=>A.esc(x.status)},{t:'Preço-base',num:true,r:x=>A.moeda(x.resultado.preco_base)},{t:'CBS',num:true,r:x=>A.moeda(x.resultado.cbs)},{t:'Preço final',num:true,r:x=>A.moeda(x.resultado.preco_final)},{t:'Calculado em',r:x=>A.esc(x.calculado_em)}],r.versoes)});});
  el.querySelectorAll('[data-cenarios-prec]').forEach(b=>b.onclick=async()=>{try{const r=await A.api(`/precificacao/itens/${b.dataset.cenariosPrec}/cenarios`,{metodo:'POST',corpo:{}});A.modal({titulo:'Cenários comerciais A–H',largura:1000,corpo:A.tabela([{t:'Cenário',r:x=>`<b>${A.esc(x.codigo)} — ${A.esc(x.nome)}</b>`},{t:'Situação',r:x=>A.esc(x.status)},{t:'Preço final',num:true,r:x=>x.resultado?A.moeda(x.resultado.preco_final):'—'},{t:'Variação',num:true,r:x=>x.variacao_preco==null?'—':A.moeda(x.variacao_preco)},{t:'Observação',r:x=>A.esc(x.motivo||'')}],r.cenarios)});}catch(e){A.toast(e.message,'erro');}});
}
  ...
  el.querySelectorAll('[data-part]').forEach((b) => { b.onclick = () => {
    ...
    A.modal({ titulo: `Participantes — ${t.titulo}`, largura: 720, confirmar: null,
      corpo: `<div class="aviso ${t.trilha === 'workshop_boas_praticas' ? 'bom' : ''}"><b>${t.trilha === 'workshop_boas_praticas' ? 'Turma compartilhada.' : 'Turma exclusiva.'}</b> ${t.trilha === 'workshop_boas_praticas' ? 'Cada participante deve ser vinculado à respectiva empresa.' : 'Os participantes são vinculados automaticamente a esta empresa.'}</div><div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">
        ...
        <div style="margin-top:10px"><button class="btn vazio pq" onclick="App.baixarArquivo('/modelos/participantes').catch(e=>App.toast(e.message,'erro'))">Baixar modelo</button></div>
...
Telas.servicos = async (el) => {
  ...
  el.innerHTML = cab('Gestão do produto', 'Serviços e combos',
    'Defina os serviços e combos aplicáveis à empresa selecionada. A aprovação posterior congela o escopo e libera os módulos correspondentes.') +
...
Telas.questor = async (el) => {
  ...
  document.getElementById('solicitarLoteQuestor').onclick = () => {
    ...
    setTimeout(()=>{
      const modal=document.getElementById('selecionarTodasEmpresasQuestor'); if(modal) modal.onclick=()=>document.querySelectorAll('input[name="questor_lote_empresa"]:not(:disabled)').forEach((x)=>{x.checked=true;});
      const atualizarHistorico=async()=>{try{const tipo=document.querySelector('[name="questor_lote_tipo"]')?.value; if(!tipo)return;const r=await A.api(`/questor/lotes/status?tipo=${encodeURIComponent(tipo)}`);const porEmpresa=new Map((r.historico||[]).map((x)=>[String(x.empresa_id),x]));document.querySelectorAll('[data-questor-lote-status]').forEach((elStatus)=>{const h=porEmpresa.get(elStatus.dataset.questorLoteStatus);if(!h){elStatus.textContent='Ainda não há busca registrada.';return;}const quantidade=h.quantidade===null?'':` · ${h.quantidade} registro(s) processado(s)`;const periodo=h.inicio&&h.fim?` · ${h.inicio} a ${h.fim}`:'';const quando=h.executado_em||h.criado_em||'';elStatus.textContent=`Última busca: ${h.status}${periodo}${quantidade}${quando?` · ${quando}`:''}`;});}catch(_){document.querySelectorAll('[data-questor-lote-status]').forEach((x)=>{x.textContent='Histórico indisponível neste momento.';});}};
      document.querySelector('[name="questor_lote_tipo"]')?.addEventListener('change',atualizarHistorico); atualizarHistorico();
    },0);
  };
  document.getElementById('importarApuracaoQuestor').onclick = async (evento) => {
    ...
    try {
      ...
      const origemParametros = r.perfil_parametros === 'DESCOBERTO_NO_QUESTOR' ? 'Configuração confirmada para esta instalação Questor.' : 'Usando a configuração compatível desta instalação Questor.';
