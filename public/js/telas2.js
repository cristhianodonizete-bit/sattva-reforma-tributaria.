...
async function telaPrecificacaoVersionada(el) {
  ...
  el.querySelectorAll('[data-calcular-prec]').forEach(b=>b.onclick=()=>A.modal({titulo:'Calcular versão',descricao:'A CBS deve ter origem identificável no motor tributário. Créditos globais só entram quando possuem rateio explícito.',corpo:A.campo('custo_bruto','Custo bruto','0','number')+A.campo('credito_direto','Crédito direto','0','number')+A.selecao('aplicar_creditos_globais','Aplicar créditos globais rateados',[{v:'false',t:'Não'},{v:'true',t:'Sim'}])+A.campo('credito_global_rateado','Crédito global rateado manual (se não aplicar automático)','0','number')+A.campo('aliquota_efetiva_cbs','Alíquota efetiva CBS (ex.: 0,09)','0','number')+A.selecao('tratamento','Tratamento',[{v:'INTEGRAL',t:'Integral'},{v:'REDUCAO_60',t:'Redução de 60%'},{v:'REDUCAO_30',t:'Redução de 30%'},{v:'ALIQUOTA_ZERO',t:'Alíquota zero'},{v:'ISENCAO',t:'Isenção'}])+A.area('evidencia_fiscal','Evidência do motor fiscal',''),aoConfirmar:async dados=>{const t={tratamento:dados.tratamento,aliquota_efetiva_cbs:Number(dados.aliquota_efetiva_cbs)||0,preserva_credito:dados.tratamento==='ALIQUOTA_ZERO',estorna_credito:dados.tratamento==='ISENCAO'};await A.api(`/precificacao/itens/${b.dataset.calcularPrec}/calcular`,{metodo:'POST',corpo:{...dados,aplicar_creditos_globais:dados.aplicar_creditos_globais==='true',evidencia_fiscal:dados.evidencia_fiscal,tratamentos:[t]}});telaPrecificacaoVersionada(el);}}));
