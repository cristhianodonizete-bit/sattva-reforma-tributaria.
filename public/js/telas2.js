...
async function telaPrecificacaoVersionada(el) {
  ...
  el.innerHTML=cab('Módulo 2','Precificação versionada','Importe a planilha para cadastrar vários itens de uma vez. “Novo item” é destinado a exceções e ajustes pontuais. Cada cálculo gera uma versão auditável.',`<button class="btn" id="importarPlanilhaPrec">Importar planilha</button><button class="btn vazio" id="baixarModeloPrecVersionada">Baixar modelo XLSX</button><button class="btn vazio" id="novoItemPrec">Novo item</button><button class="btn vazio" id="creditosGlobaisPrec">Créditos globais</button><button class="btn vazio" id="importacoesPrec">Lotes de importação</button><button class="btn vazio" id="validarLotePrec">Validar calculados</button><button class="btn vazio" id="aprovarLotePrec">Aprovar validados</button><button class="btn vazio" id="reprocessarTodosPrec">Reprocessar calculados</button><button class="btn vazio" id="voltarPrecVersionada">Voltar</button>`) +
