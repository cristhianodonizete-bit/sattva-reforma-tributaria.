...
async function telaPrecificacaoVersionada(el) {
  const [d,premissasDados,conferencia]=await Promise.all([A.api(`/empresas/${S.empresaId}/precificacao/itens`),A.api(`/empresas/${S.empresaId}/precificacao/premissas-comerciais`),A.api(`/empresas/${S.empresaId}/precificacao/conferencia`)]); const todos=d.itens || []; const premissas=premissasDados.premissas||[]; const ep=estadoPrecificacaoVersionada;
  ...
  document.getElementById('premissasComerciaisPrec').onclick=()=>{const abrir=()=>A.modal({titulo:'Premissas comerciais',largura:900,corpo:`<p class="mini">As premissas ficam na empresa e podem ser reutilizadas em simulações, sem alterar a base operacional nem versões já calculadas.</p>${A.tabela([{t:'Cenário',r:x=>A.esc(x.nome)},{t:'Margem de contribuição',num:true,r:x=>A.pct(x.margem_contribuicao)},{t:'Percentuais por dentro',num:true,r:x=>A.pct(x.percentuais_por_dentro)},{t:'Padrão',r:x=>x.padrao?'Sim':'—'}],premissas,{vazio:'Nenhuma premissa cadastrada.'})}<button class="btn" id="novaPremissaComercial">Nova premissa</button>`});const modal=abrir();modal.fundo.querySelector('#novaPremissaComercial').onclick=()=>A.modal({titulo:'Nova premissa comercial',corpo:A.campo('nome','Nome do cenário','')+A.campo('margem_contribuicao','Margem de contribuição (ex.: 0,20)','0','number')+A.campo('percentuais_por_dentro','Percentuais por dentro (ex.: 0,10)','0','number')+A.selecao('padrao','Usar como padrão',[{v:'false',t:'Não'},{v:'true',t:'Sim'}]),aoConfirmar:async dados=>{await A.api(`/empresas/${S.empresaId}/precificacao/premissas-comerciais`,{metodo:'POST',corpo:{...dados,padrao:dados.padrao==='true'}});telaPrecificacaoVersionada(el);}});};
