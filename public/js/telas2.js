...
async function telaPrecificacaoVersionada(el) {
  ...
  el.innerHTML=cab('Módulo 2','Precificação versionada','Cada cálculo gera uma versão auditável. A alíquota da CBS é sempre acompanhada da evidência do motor tributário.',`<button class="btn" id="novoItemPrec">Novo item</button><button class="btn vazio" id="creditosGlobaisPrec">Créditos globais</button><button class="btn vazio" id="reprocessarTodosPrec">Reprocessar calculados</button><button class="btn vazio" id="voltarPrecVersionada">Voltar</button>`) +
    `<div class="grade g3">${A.kpi('Itens ativos',itens.filter(x=>x.ativo).length)}${A.kpi('Versões calculadas',itens.filter(x=>x.calculo_id).length)}${A.kpi('Vigentes',itens.filter(x=>x.calculo_status==='VIGENT').length)}</div><div class="cartao"><h2>Itens e cálculos</h2>${A.tabela([
      ...
      {t:'Ações',r:x=>`<button class="btn pq vazio" data-calcular-prec="${x.id}">Calcular</button>${x.calculo_id?` <button class="btn pq vazio" data-cenarios-prec="${x.id}">Cenários</button>${x.calculo_status==='CALCULATED'?` <button class="btn pq" data-status-prec="${x.calculo_id}" data-alvo="VALIDATION">Validar</button>`:''}${x.calculo_status==='VALIDATION'?` <button class="btn pq" data-status-prec="${x.calculo_id}" data-alvo="APPROVED">Aprovar</button>`:''}${x.calculo_status==='APPROVED'?` <button class="btn pq" data-status-prec="${x.calculo_id}" data-alvo="VIGENT">Vigorar</button>`:''}`:''}`},
