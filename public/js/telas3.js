...
// Visão inicial: não depende da empresa selecionada. Ela é separada da tela
  ...
  const carregar = async () => {
    ...
    el.innerHTML = cab('GOVERNANÇA NORMATIVA', 'Atualizações da Reforma', 'Registro de fontes oficiais e avaliação humana. Nenhuma atualização altera regras ou cálculos automaticamente.', '<button class="btn" id="novaAtualizacaoReforma">Registrar atualização</button>') +
      ...
      `<section class="cartao" style="margin-top:16px"><div class="aviso"><b>Monitor diário ativo</b><br>Fontes oficiais são acompanhadas fora do motor. Registre ou revise aqui apenas após leitura e decisão humana.</div>${A.tabela([
        {t:'Atualização e resumo',r:x=>`<b>${A.esc(x.titulo)}</b><div class="mini">${A.esc(x.tema)} · ${A.esc(x.data_publicacao || 'data não informada')}</div><p class="mini" style="margin:8px 0 0;max-width:520px;line-height:1.45">${A.esc(x.resumo || 'Ainda não há resumo disponível. Abra a revisão e registre a síntese da fonte oficial.')}</p>${x.modulos_afetados ? `<div class="mini" style="margin-top:6px"><b>Módulos envolvidos:</b> ${A.esc(x.modulos_afetados)}</div>` : ''}`},
        {t:'Fonte de consulta',r:x=>x.fonte_url?`<span class="mini">${A.esc(x.fonte_nome || 'Fonte oficial')}</span><br><a href="${A.esc(x.fonte_url)}" target="_blank" rel="noopener">Abrir publicação oficial ↗</a>`:A.esc(x.fonte_nome || '—')},
        ...
        {t:'',r:x=>`<button class="btn pq vazio" data-ler-fonte="${x.id}">Ler e resumir</button><button class="btn pq vazio" data-revisar-atualizacao="${x.id}" style="margin-top:6px">Revisar</button>`},
      ...
      `<section class="cartao" style="margin-top:16px"><h2>Fontes monitoradas</h2>${A.tabela([
        {t:'Fonte',r:x=>`<a href="${A.esc(x.fonte_url)}" target="_blank" rel="noopener">${A.esc(x.fonte_nome)}</a>`},
        {t:'Última consulta',r:x=>A.esc(x.ultima_consulta_em || 'Ainda não consultada')},
        {t:'Resultado',r:x=>x.ultimo_erro ? `<span class="tag n">Falhou</span><div class="mini">${A.esc(x.ultimo_erro)}</div>` : x.ultimo_sucesso_em ? '<span class="tag c">Consultada</span>' : '<span class="tag b">Aguardando</span>'},
      ],fontes,{vazio:'O monitor ainda está criando a linha de base das fontes oficiais.',desc:'Acompanhe aqui a execução diária. Uma falha fica visível mesmo quando nenhuma notícia é registrada.'})}</section>`;
    ...
    el.querySelectorAll('[data-ler-fonte]').forEach((b)=>b.onclick=async()=>{const x=itens.find(i=>Number(i.id)===Number(b.dataset.lerFonte));try{b.disabled=true;b.textContent='Lendo fonte…';await A.api(`/atualizacoes-reforma/${x.id}/ler-fonte`,{metodo:'POST',corpo:{}});A.toast('Fonte oficial lida e resumo atualizado.','ok');await carregar();}catch(e){A.toast(e.message||'Não foi possível ler a fonte oficial.','erro');b.disabled=false;b.textContent='Ler e resumir';}});
