...
Telas.dashboardOperacao = async (el) => {
  ...
  const renderAgenda = () => {
    ...
    el.querySelector('#listaAgenda').innerHTML = visiveis.length ? `<div class="agenda-marcos">${visiveis.map((m) => `<div class="agenda-marco${m.atrasado ? ' atrasado' : ''}"><div class="agenda-data"><b>${A.esc(m.data)}</b>${m.atrasado ? '<small>Atrasado</small>' : ''}</div><div class="agenda-conteudo"><b>${A.esc(m.titulo)}${m.envolveCliente ? ' · envolve cliente' : ''}</b><span>${A.esc(m.empresa)}${m.etapa ? ` · ${A.esc(m.etapa)}` : ''}${m.responsavelSattva ? ` · Sattva: ${A.esc(m.responsavelSattva)}` : ''}${m.responsavelCliente ? ` · Cliente: ${A.esc(m.responsavelCliente)}` : ''}</span>${m.pendenciaCliente ? `<small class="agenda-pendencia">Pendência: ${A.esc(m.pendenciaCliente)}</small>` : ''}</div><button class="btn pq vazio" data-ir-projeto="${m.empresaId || ''}">Abrir</button></div>`).join('')}</div>` : A.vazio('Nenhum marco com data foi registrado.', 'Inclua prazos nas tarefas ou competências de acompanhamento.');
...
Telas.acessos = async (el) => {
  ...
  const empresasDoUsuario = (usuarioId) => new Set(d.vinculos.filter((v) => v.usuario_id === usuarioId).map((v) => String(v.empresa_id)));
  const abrirUsuario = (usuario = null) => {
    const vinculadas = empresasDoUsuario(usuario?.id);
    const empresas = d.empresas.length ? `<div class="empresas-acesso">${d.empresas.map((empresa) => `<label class="check"><input type="checkbox" name="empresa_${empresa.id}" ${vinculadas.has(String(empresa.id)) ? 'checked' : ''}> ${A.esc(empresa.razao_social)}</label>`).join('')}</div>` : '<p class="mini">Nenhuma empresa cadastrada para atribuição.</p>';
    A.modal({ titulo: usuario ? 'Editar usuário' : 'Novo usuário', largura: 700,
      corpo: `${A.campo('nome', 'Nome', usuario?.nome || '')}${usuario ? `<label class="campo"><span>E-mail</span><input value="${A.esc(usuario.email)}" disabled></label>` : A.campo('email', 'E-mail', '', 'email')}${usuario ? '' : A.campo('senha', 'Senha inicial', '', 'password')} ${A.selecao('perfil_acesso_id', 'Perfil de acesso', opcoesPerfil(), usuario?.perfil_acesso_id || '')}<label class="check"><input type="checkbox" name="ativo" ${usuario?.ativo !== false ? 'checked' : ''}> Usuário ativo</label><h3 class="subtitulo-modal">Empresas que este usuário pode atender</h3>${empresas}`,
