...
Telas.classificacaoFiscalComplementar = async (el) => {
  ...
  const render = () => {
    ...
    el.innerHTML = `<div class="topo"><div><div class="olho">CADASTRO COMPLEMENTAR</div><h1>Classificação Fiscal Complementar</h1><p>Responda apenas fatos materiais por empresa e produto. Esta tela não escolhe CST, alíquota ou tratamento e não reprocessa documentos.</p></div></div>
      ...
      <div class="cartao" style="margin-top:16px"><h2>NCMs históricos que exigem evidência</h2><p class="desc">Esta triagem não cria fatos nem altera CST, PIS/Cofins, CBS ou o motor. Ela só aparece quando há produto desta empresa com NCM histórico correlacionado oficialmente.</p>${historicosDaEmpresa.length ? A.tabela([
        {t:'Produto',r:x=>x.produtos_empresa.map(p=>`<b>${A.esc(p.codigo_produto_atual||`Produto #${p.id}`)}</b><div class="mini">${A.esc(p.descricao_atual||'Sem descrição')} · NCM ${A.esc(p.ncm_atual)}</div>`).join('<hr style="border:0;border-top:1px solid #eef2f6">')},
      ...
      ],historicosDaEmpresa) : A.vazio('Nenhum produto desta empresa usa os 13 NCMs históricos mapeados.', 'Quando houver um produto correspondente, a evidência será solicitada antes de qualquer decisão fiscal.')}</div>
