...
Telas.utilidadesFiscais = async (el) => {
  ...
  // Mantém a tela autocontida. Isso evita que uma alteração futura no
  // cabeçalho compartilhado impeça a consulta às fontes oficiais.
  const topoUtilidades = (olho, titulo, texto) =>
    `<div class="topo"><div><div class="olho">${olho}</div><h1>${titulo}</h1>${texto ? `<p>${texto}</p>` : ''}</div></div>`;
  ...
  const cardAnexoViii = `<article class="cartao" style="box-shadow:none"><h3>LC 116 × NBS × cIndOp × cClassTrib</h3><p class="desc">Tabela oficial do Anexo VIII do Portal Nacional da NFS-e para serviços.</p><button class="btn vazio pq" id="verAnexoViii">Ver tabela</button></article>`;
  el.innerHTML = topoUtilidades('CONSULTA OFICIAL', 'Utilidades fiscais', 'Legislação, tabelas técnicas e fontes oficiais verificadas diariamente. Uma alteração encontrada abre revisão; nunca muda o motor automaticamente.') +
    ...
    <section class="cartao" style="margin-top:16px"><h2>Tabelas</h2><p class="desc">Consulte as publicações oficiais vigentes. As versões expiradas permanecem disponíveis apenas para consulta histórica.</p><div class="grade g2">${cardTabela('cClassTrib IBS/CBS','Essencial para classificação e regras CBS/IBS.')} ${cardTabela('cCredPres','Referência para crédito presumido, quando a hipótese legal estiver configurada.')} ${cardTabela('Alíquotas CBS','Referência oficial para validar os parâmetros e a transição.')} ${cardTabela('CFOP','Apoia a natureza da operação e a conformidade documental; o motor já usa a lógica de CFOP.')} ${cardTabela('FCP por UF','Referência para reconstruir a carga atual de ICMS em operações sujeitas.')} ${cardAnexoViii}</div><h3 style="margin-top:20px">cBenef × CST por UF</h3><p class="desc">Útil para conformidade documental e reconstrução de ICMS. Não muda CBS automaticamente.</p><div class="tags" style="gap:8px">${cbenef.map(([uf,url]) => `<a class="tag" target="_blank" rel="noopener" href="${A.esc(url)}">${uf} ↗</a>`).join('')}</div></section>
  ...
  el.querySelector('#verAnexoViii')?.addEventListener('click', () => A.modal({ titulo:'Anexo VIII — LC 116 × NBS × cIndOp × cClassTrib', descricao:'Tabela lida diretamente da publicação oficial do Portal Nacional da NFS-e.', largura:1200, confirmar:null, corpo:'<div style="display:flex;gap:8px;margin-bottom:12px"><input id="buscaAnexoViii" placeholder="Buscar LC 116, NBS, cIndOp ou cClassTrib"><button class="btn" id="buscarAnexoViii">Buscar</button></div><div id="resultadoAnexoViii" class="carregando">Carregando tabela oficial…</div>', aoAbrir: async (modal) => { const resultado=modal.querySelector('#resultadoAnexoViii'); const carregar=async()=>{resultado.innerHTML='<div class="carregando">Consultando publicação oficial…</div>';try{const r=await A.api(`/utilidades-fiscais/anexo-viii?busca=${encodeURIComponent(modal.querySelector('#buscaAnexoViii').value)}`);resultado.innerHTML=`<p class="mini">${r.total} linha(s) encontradas · aba ${A.esc(r.aba)}</p>${A.tabela([{t:'LC 116',r:x=>A.esc(x.lc116)},{t:'NBS',r:x=>A.esc(x.nbs)},{t:'cIndOp',r:x=>A.esc(x.indop)},{t:'cClassTrib',r:x=>A.esc(x.cclasstrib)},{t:'Descrição',r:x=>A.esc(x.descricao_nbs || x.descricao_lc116)}],r.itens,{vazio:'Nenhuma linha encontrada.'})}`;}catch(e){resultado.innerHTML=`<div class="aviso alto">${A.esc(e.message)}</div>`;}};modal.querySelector('#buscarAnexoViii').onclick=carregar;modal.querySelector('#buscaAnexoViii').onkeydown=e=>{if(e.key==='Enter')carregar();};await carregar();} }));
};
})();
