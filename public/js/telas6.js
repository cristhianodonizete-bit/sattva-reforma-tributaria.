...
// -------------------------------------------------------------------- CFOP
function cfop(box, d) {
  const naturezas = ['venda', 'aquisicao', 'devolucao', 'devolucao_venda', 'devolucao_fornecedor', 'remessa', 'transferencia',
    'exportacao', 'importacao', 'ativo_consumo', 'outra_saida'];
  const rotuloNatureza = { devolucao_venda:'Devolução de venda — reduz faturamento', devolucao_fornecedor:'Devolução a fornecedor — reduz compras' };
  const opcaoNatureza = (n) => `<option value="${n}">${A.esc(rotuloNatureza[n] || n)}</option>`;
  ...
  box.innerHTML = `<div class="aviso"><b>Catálogo operacional de CFOP</b> Esta é a mesma fonte usada para decidir se uma operação compõe a receita analisada. CFOP de produto só entra por classificação positiva; código sem regra fica fora do faturamento e é sinalizado para revisão.
    ...
    <div class="cartao"><h2>Cadastrar regra de CFOP</h2><p class="desc">Informe três dígitos para uma regra espelhada (ex.: 949 cobre 5.949 e 6.949) ou quatro dígitos para uma regra específica. Use “devolucao_venda” para reduzir faturamento e “devolucao_fornecedor” para reduzir compras.</p>
      <div class="grade g3"><label class="campo"><span>CFOP (3 ou 4 dígitos)</span><input id="novoCfopCodigo" inputmode="numeric" maxlength="4" placeholder="Ex.: 1202"></label><label class="campo"><span>Natureza</span><select id="novoCfopNatureza">${naturezas.map(opcaoNatureza).join('')}</select></label><label class="campo"><span>Compõe receita?</span><select id="novoCfopReceita"><option value="nao">Não</option><option value="sim">Sim</option></select></label></div>
    ...
    ${porPrioridade.map((lista, i) => lista.length ? `<div class="cartao">
      ...
      ${A.tabela([
        ...
        { t: 'Natureza da operação', r: (c) => `<select data-f="nat" data-i="${c.id}">
            ${naturezas.map((n) => `<option value="${n}" ${c.natureza === n ? 'selected' : ''}>${A.esc(rotuloNatureza[n] || n)}</option>`).join('')}</select>` },
  ...
  document.getElementById('salvarNovoCfop').onclick = async () => {
    ...
    if (![3,4].includes(cfop.length)) { A.toast('Informe três ou quatro dígitos do CFOP.', 'erro'); return; }
    try {
      ...
      A.toast(`Regra ${cfop} cadastrada e publicada na fonte compartilhada.`, 'ok'); A.ir('configuracoes');
