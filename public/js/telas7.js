...
async function catalogoFiscal(el) {
  const estado = S.cache.catalogoFiscal || (S.cache.catalogoFiscal = { tipo: 'referencia_ncm', busca: '', pagina: 1 });
  const carregar = async () => {
    ...
    const colunas = !operacional ? [
      ...
      ...(estado.tipo === 'referencia_ncm' ? [{ t: 'Matriz operacional', r: (x) => x.possui_regra_operacional ? '<span class="tag c">Regra homologada</span>' : '<span class="tag n">Sem regra operacional</span>' }] : []),
    ...
    el.innerHTML = `<div class="topo"><div><div class="olho">Bases fiscais</div><h1>Catálogo fiscal</h1><p>O catálogo abre na nomenclatura oficial completa. Regras operacionais são uma camada separada e só entram no motor após homologação.</p></div><button class="btn vazio" id="catalogoExportar">Baixar Excel completo</button></div>${resumoOficial}<div class="cartao" style="margin-top:16px"><div class="filtros-carteira"><label>Camada consultada<select id="catalogoTipo"><optgroup label="Referência oficial (sem regra tributária)"><option value="referencia_ncm" ${estado.tipo === 'referencia_ncm' ? 'selected' : ''}>NCM oficial completa</option><option value="referencia_nbs" ${estado.tipo === 'referencia_nbs' ? 'selected' : ''}>NBS oficial completa</option><option value="referencia_lc116" ${estado.tipo === 'referencia_lc116' ? 'selected' : ''}>Lista oficial LC 116</option></optgroup><optgroup label="Regras operacionais do motor"><option value="ncm" ${estado.tipo === 'ncm' ? 'selected' : ''}>Mercadorias — NCM</option><option value="servicos" ${estado.tipo === 'servicos' ? 'selected' : ''}>Serviços — NBS / LC 116</option></optgroup></select></label><label style="flex:1">Buscar código ou descrição<input id="catalogoBusca" value="${A.esc(estado.busca)}" placeholder="Ex.: 30049099, 1.1502.10.00 ou medicamento"></label><button class="btn" id="catalogoBuscar">Buscar</button></div>${explicacao}<div style="margin-top:14px">${A.tabela(colunas, r.itens, { vazio: 'Nenhum registro encontrado.' })}</div><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:14px"><span class="mini">Página ${r.pagina} de ${paginas}</span><div style="display:flex;gap:8px"><button class="btn pq vazio" id="catalogoAnterior" ${r.pagina <= 1 ? 'disabled' : ''}>Anterior</button><button class="btn pq vazio" id="catalogoProximo" ${r.pagina >= paginas ? 'disabled' : ''}>Próxima</button></div></div></div>`;
