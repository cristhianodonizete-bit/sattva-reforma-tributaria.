...
async function main() {
  ...
  const rastreabilidade = resultados.map((r) => {
    ...
    return {
      ...
      'PIS retirado': moeda(rec.tributosAtuais?.pis),
      'COFINS retirado': moeda(rec.tributosAtuais?.cofins),
      'ISS retirado': moeda(rec.tributosAtuais?.iss),
      'ICMS retirado': moeda(rec.tributosAtuais?.icms),
  ...
  const resumo = [
    ...
    { 'Seção': 'SAÍDAS', 'Indicador': 'Base econômica', 'Valor': total(saidas, 'base_economica'), 'Origem': 'somente saídas' },
    ...
    { 'Seção': 'ENTRADAS', 'Indicador': 'Base econômica', 'Valor': total(entradas, 'base_economica'), 'Origem': 'somente entradas' },
