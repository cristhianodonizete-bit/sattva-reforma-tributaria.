...
async function espelharEmpresas(empresas, remoto = supabase) {
  ...
  for (const empresa of empresas) {
    const linha = { origem_local_id: empresa.id, cnpj: empresa.cnpj, razao_social: empresa.razao_social, nome_fantasia: empresa.nome_fantasia || null,
      ...
      atividade: empresa.atividade || null, cnaes_secundarios: empresa.cnaes_secundarios || null, data_abertura: empresa.data_abertura || null };
...
async function executar() {
  const empresas = db.prepare('SELECT id, cnpj, razao_social, nome_fantasia, regime, uf, municipio, cnae, atividade, cnaes_secundarios, data_abertura FROM empresas').all();
