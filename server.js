...
function prepararMotor() {
  try {
    ...
    // Um deploy não é autorização para calcular empresa alguma. O motor só
    // pode ser disparado por importação, mudança de regra ou ação explícita.
...
async function iniciarOperacao() {
  ...
  try {
    ...
    if (operacao.ativo()) {
      ...
      // Jobs sobrevivem a reinícios, mas o boot apenas recupera o estado.
      // Processar uma fila durante a carga-base concorria com a leitura dos
