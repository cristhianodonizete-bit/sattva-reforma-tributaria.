...
const COLUNAS_NOVAS = {
  ...
  param_cfop: {
    ...
    // A classificação de receita é uma decisão auditável, distinta da mera
    // natureza fiscal. Não se presume venda pelo primeiro dígito do CFOP.
    compoe_receita: 'INTEGER', fonte: 'TEXT', versao: 'TEXT', vigencia_inicio: 'TEXT', ativo: 'INTEGER DEFAULT 1',
  },
...
CREATE TABLE IF NOT EXISTS empresas (
  ...
  sensibilidade_credito TEXT DEFAULT 'indeterminado',
  sensibilidade_origem TEXT DEFAULT 'nao_definido',
  regime_reconhecimento_simples TEXT DEFAULT 'competencia',
  uf TEXT, municipio TEXT, cnae TEXT, atividade TEXT, cnaes_secundarios TEXT, data_abertura TEXT,
...
CREATE TABLE IF NOT EXISTS pricing_base_operacional (
  ...
  classificacao_compra TEXT, classificacao_venda TEXT, classificacao_compra_detalhe TEXT, classificacao_venda_detalhe TEXT,
...
CREATE TABLE IF NOT EXISTS pricing_base_estrutura (
  ...
  codigo_componente TEXT NOT NULL, descricao TEXT NOT NULL, tipo_componente TEXT NOT NULL,
  valor REAL NOT NULL DEFAULT 0, pis REAL, cofins REAL, fornecedor_cnpj TEXT, regime_fornecedor TEXT, ncm TEXT, nbs TEXT, lc116 TEXT, classificacao TEXT, classificacao_detalhe TEXT,
...
CREATE TABLE IF NOT EXISTS cnpj_cache (
  ...
  cnae TEXT, cnae_descricao TEXT, cnaes_secundarios TEXT, uf TEXT, municipio TEXT, data_abertura TEXT,
...
CREATE TABLE IF NOT EXISTS especialista_fiscal_interacoes (
  ...
  uso_json TEXT NOT NULL DEFAULT '{}',
  criado_em TEXT NOT NULL DEFAULT (datetime('now','localtime'))
);
CREATE INDEX IF NOT EXISTS ix_especialista_fiscal_interacoes ON especialista_fiscal_interacoes(empresa_id,id DESC);
...
if (db.prepare('SELECT COUNT(*) c FROM param_cfop').get().c === 0) {
  ...
  db.transaction(() => {
    ...
    for (const [nat, lista] of Object.entries(G)) lista.forEach((g) => ins.run(g, null, nat, 2, '', 0, 'Tabela CFOP', '2023.002', '2023-04-24'));
    // Não existe fallback 5xxx/6xxx = venda: cada código de venda entra na
    // lista positiva abaixo, para impedir que remessas e outras saídas
    // contaminem o faturamento.
  })();
}

// Catálogo-base, versionado, de CFOPs que representam venda de
// mercadorias. O espelho 5/6 é inserido em conjunto. Códigos não presentes
