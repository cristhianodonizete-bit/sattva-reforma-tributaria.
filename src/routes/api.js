...
router.post('/empresas/:id/precificacao/base-operacional/importar', upload.single('arquivo'), async (req,res) => {
  try {
    ...
    const classificarBase=(dados,sentido)=>{
      ...
      return { categoria, detalhe:JSON.stringify({status:resultado.status,cst:resultado.cst,cclasstrib:resultado.cclasstrib,tratamento:resultado.tratamento,reducao:resultado.reducao,origem_regra:resultado.origemRegra,fundamentos:resultado.fundamentos}) };
    };
