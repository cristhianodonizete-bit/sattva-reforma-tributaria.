...
router.get('/empresas/:id/cadeia/:tipo', async (req, res) => {
  try {
    ...
    if (empresa.regime === 'simples_nacional') await estadoLeituraEmpresa.atualizarComSeguranca(db, empresaId, ['pgdas'], () => pgdasCompartilhado.restaurar(empresa.id), { motivo:'PGDAS confirmado restaurado para a cadeia' });
...
router.get('/empresas/:id/precificacao/base-operacional/template', (_req,res) => {
  ...
  aba('Base_Operacional',['codigo','descricao','modelo','fornecedor_cnpj','pis','cofins','valor_aquisicao','prazo_depreciacao_meses','valor_residual','ncm_compra','nbs_compra','lc116_compra','ncm_venda','nbs_venda','lc116_venda','evidencia'],['LOC-001','Equipamento locado','LOCACAO','00000000000000',0,0,120000,60,20000,'84713012','','','84713012','','','Contrato e NF de aquisição']);
  aba('Estrutura_Itens',['codigo_item','codigo_componente','descricao','tipo_componente','valor','pis','cofins','fornecedor_cnpj','ncm','nbs','lc116','evidencia'],['PROD-001','MP-001','Matéria-prima','MATERIA_PRIMA',100,1.65,7.6,'00000000000000','11010010','','','NF de entrada']);
...
router.get('/empresas/:id/precificacao/base-operacional', (req,res) => {
  try {
    const bases=db.prepare('SELECT * FROM pricing_base_operacional WHERE empresa_id=? AND ativo=1 ORDER BY codigo').all(Number(req.params.id));
    const componentes=db.prepare(`SELECT e.* FROM pricing_base_estrutura e JOIN pricing_base_operacional b ON b.id=e.base_operacional_id WHERE b.empresa_id=? AND b.ativo=1 ORDER BY e.codigo_componente`).all(Number(req.params.id));
    const detalhe=x=>{try{return x?JSON.parse(x):null;}catch(_){return null;}};
    ok(res,{bases:bases.map(x=>({...x,classificacao_compra_detalhe:detalhe(x.classificacao_compra_detalhe),classificacao_venda_detalhe:detalhe(x.classificacao_venda_detalhe)})),componentes:componentes.map(x=>({...x,classificacao_detalhe:detalhe(x.classificacao_detalhe)}))});
  } catch(e){erro(res,e);}
});
router.post('/empresas/:id/precificacao/base-operacional/importar', upload.single('arquivo'), async (req,res) => {
  try {
    ...
    const fiscalOk=x=>String(x.ncm||'').replace(/\D/g,'').length===8 || (String(x.lc116||'').trim()&&String(x.nbs||'').replace(/\D/g,'').length>=6);
    bases.forEach((x,i)=>{if(!x.codigo||!x.descricao||!modelos.has(String(x.modelo||'').toUpperCase()))erros.push({aba:'Base_Operacional',linha:i+2,erro:'Código, descrição e modelo válido são obrigatórios.'});if(!fiscalOk({ncm:x.ncm_compra,nbs:x.nbs_compra,lc116:x.lc116_compra})||!fiscalOk({ncm:x.ncm_venda,nbs:x.nbs_venda,lc116:x.lc116_venda}))erros.push({aba:'Base_Operacional',linha:i+2,erro:'Compra e venda exigem NCM de 8 dígitos ou, para serviço, LC 116 e NBS.'});if(String(x.modelo||'').toUpperCase()==='LOCACAO'&&(!(Number(x.valor_aquisicao)>0)||!(Number(x.prazo_depreciacao_meses)>0)))erros.push({aba:'Base_Operacional',linha:i+2,erro:'Locação exige valor do equipamento e prazo de depreciação.'});});
    estrutura.forEach((x,i)=>{if(!x.codigo_item||!x.codigo_componente||!x.descricao||Number(x.valor)<0||!fiscalOk(x))erros.push({aba:'Estrutura_Itens',linha:i+2,erro:'Componente exige identificação e NCM ou LC 116 + NBS.'});}); if(erros.length)return ok(res,{importado:false,erros});
    const normalizarCnpj=x=>String(x||'').replace(/\D/g,'');
    const parceiro=db.prepare('SELECT regime FROM parceiros WHERE empresa_id=? AND cnpj=? LIMIT 1');
    const cnpjs=[...bases,...estrutura].map(x=>normalizarCnpj(x.fornecedor_cnpj)).filter(Boolean);
    const compartilhados=await baseRegime.consultarCompartilhada(cnpjs,2024);
    // A planilha nunca decide o regime. A precedência é cadastro compartilhado,
    // base nacional 2024 compartilhada, base local 2024 e, por último, regular.
    const resolverRegime=cnpj=>parceiro.get(empresaId,cnpj)?.regime||compartilhados.get(cnpj)?.regime||compartilhados.get(cnpj.slice(0,8))?.regime||baseRegime.consultar(cnpj,2024)?.regime||'regime_regular';
    // Não há perfil de cliente nesta chamada: o tratamento pertence ao item e
    // somente fica pendente quando o próprio catálogo exigir condição adicional.
    const classificarBase=(dados,sentido)=>{
      const resultado=classificarItemFiscal({ ncm:dados.ncm||'', nbs:dados.nbs||'', lc116:dados.lc116||'', origem:'base_operacional', modelo_documento_fiscal:dados.ncm?'nfe':'nfse' },{empresa:{id:empresaId},sentido});
      let categoria='PENDENTE_DE_ENQUADRAMENTO';
      if(resultado.status==='CLASSIFICADO') categoria=(resultado.reducao==='imune'||String(resultado.cst||'').startsWith('4'))?'ISENCAO':(resultado.reducao!=='integral'||Number(resultado.reducaoIbs)||Number(resultado.reducaoCbs))?'REDUCAO':'TRIBUTACAO_INTEGRAL';
      return { categoria, detalhe:JSON.stringify({status:resultado.status,cst:resultado.cst,cclasstrib:resultado.cclasstrib,tratamento:resultado.tratamento,reducao:resultado.reducao,reducaoIbs:resultado.reducaoIbs,reducaoCbs:resultado.reducaoCbs,origem_regra:resultado.origemRegra,fundamentos:resultado.fundamentos}) };
    };
    const basesClassificadas=bases.map(x=>({...x, compra:classificarBase({ncm:x.ncm_compra,nbs:x.nbs_compra,lc116:x.lc116_compra},'entrada'), venda:classificarBase({ncm:x.ncm_venda,nbs:x.nbs_venda,lc116:x.lc116_venda},'saida')}));
    const estruturaClassificada=estrutura.map(x=>({...x, classificacao:classificarBase(x,'entrada')}));
    const upsert=db.prepare(`INSERT INTO pricing_base_operacional (empresa_id,codigo,descricao,modelo,ncm_compra,nbs_compra,lc116_compra,ncm_venda,nbs_venda,lc116_venda,classificacao_compra,classificacao_venda,classificacao_compra_detalhe,classificacao_venda_detalhe,fornecedor_cnpj,regime_fornecedor,pis,cofins,valor_aquisicao,prazo_depreciacao_meses,valor_residual,evidencia) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(empresa_id,codigo,modelo) DO UPDATE SET descricao=excluded.descricao,ncm_compra=excluded.ncm_compra,nbs_compra=excluded.nbs_compra,lc116_compra=excluded.lc116_compra,ncm_venda=excluded.ncm_venda,nbs_venda=excluded.nbs_venda,lc116_venda=excluded.lc116_venda,classificacao_compra=excluded.classificacao_compra,classificacao_venda=excluded.classificacao_venda,classificacao_compra_detalhe=excluded.classificacao_compra_detalhe,classificacao_venda_detalhe=excluded.classificacao_venda_detalhe,fornecedor_cnpj=excluded.fornecedor_cnpj,regime_fornecedor=excluded.regime_fornecedor,pis=excluded.pis,cofins=excluded.cofins,valor_aquisicao=excluded.valor_aquisicao,prazo_depreciacao_meses=excluded.prazo_depreciacao_meses,valor_residual=excluded.valor_residual,evidencia=excluded.evidencia,atualizado_em=datetime('now','localtime')`);
    db.transaction(()=>{const ids=new Map();basesClassificadas.forEach(x=>{const cnpj=normalizarCnpj(x.fornecedor_cnpj);upsert.run(empresaId,x.codigo,x.descricao,String(x.modelo).toUpperCase(),x.ncm_compra||'',x.nbs_compra||'',x.lc116_compra||'',x.ncm_venda||'',x.nbs_venda||'',x.lc116_venda||'',x.compra.categoria,x.venda.categoria,x.compra.detalhe,x.venda.detalhe,cnpj,resolverRegime(cnpj),Number(x.pis)||0,Number(x.cofins)||0,Number(x.valor_aquisicao)||0,Number(x.prazo_depreciacao_meses)||null,Number(x.valor_residual)||0,x.evidencia||'');const id=db.prepare('SELECT id FROM pricing_base_operacional WHERE empresa_id=? AND codigo=? AND modelo=?').get(empresaId,x.codigo,String(x.modelo).toUpperCase()).id;ids.set(x.codigo,id);});const ins=db.prepare('INSERT INTO pricing_base_estrutura (base_operacional_id,codigo_componente,descricao,tipo_componente,valor,pis,cofins,fornecedor_cnpj,regime_fornecedor,ncm,nbs,lc116,classificacao,classificacao_detalhe,evidencia) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');for(const id of ids.values())db.prepare('DELETE FROM pricing_base_estrutura WHERE base_operacional_id=?').run(id);estruturaClassificada.forEach(x=>{const cnpj=normalizarCnpj(x.fornecedor_cnpj);ins.run(ids.get(x.codigo_item),x.codigo_componente,x.descricao,x.tipo_componente,Number(x.valor)||0,Number(x.pis)||0,Number(x.cofins)||0,cnpj,resolverRegime(cnpj),x.ncm||'',x.nbs||'',x.lc116||'',x.classificacao.categoria,x.classificacao.detalhe,x.evidencia||'');});
      // A base operacional já é o cadastro da empresa: cada linha cria ou atualiza
      // o item de venda correspondente. Não há redigitação item a item.
      const itemPorBase=db.prepare(`INSERT INTO pricing_itens (empresa_id,codigo,descricao,modalidade,natureza_item,ncm,nbs,lc116,origem,origem_tipo,origem_id,base_operacional_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(empresa_id,codigo,modalidade) DO UPDATE SET descricao=excluded.descricao,natureza_item=excluded.natureza_item,ncm=excluded.ncm,nbs=excluded.nbs,lc116=excluded.lc116,base_operacional_id=excluded.base_operacional_id,origem='BASE_OPERACIONAL',origem_tipo='pricing_base_operacional',origem_id=excluded.base_operacional_id,atualizado_em=datetime('now','localtime')`);
      const modalidadeItem={LOCACAO:'LOCACAO',REVENDA_SERVICO:'REVENDA',PRODUCAO_COMPOSICAO_SERVICO:'PRODUCAO_COMPOSICAO',MISTO_CONTRATO:'MISTO_CONTRATO'};
      basesClassificadas.forEach(x=>{const baseId=ids.get(x.codigo);const modelo=String(x.modelo).toUpperCase();itemPorBase.run(empresaId,x.codigo,x.descricao,modalidadeItem[modelo],x.ncm_venda?'produto':'servico',x.ncm_venda||'',x.nbs_venda||'',x.lc116_venda||'','BASE_OPERACIONAL','pricing_base_operacional',baseId,baseId);});})();
    await require('../services/operacaoCompartilhada').publicar();
    const todas=[...basesClassificadas.flatMap(x=>[x.compra,x.venda]),...estruturaClassificada.map(x=>x.classificacao)];
    const classificacoes=todas.reduce((out,x)=>({...out,[x.categoria]:(out[x.categoria]||0)+1}),{});
    ok(res,{importado:true,bases:bases.length,componentes:estrutura.length,classificacoes});
  }catch(e){erro(res,e);}
});
...
router.get('/empresas/:id/precificacao/itens', (req, res) => {
  try {
    const itens = db.prepare(`SELECT i.*, b.codigo base_operacional_codigo, b.modelo base_operacional_modelo, b.classificacao_venda base_classificacao_venda, c.id calculo_id, c.versao, c.status calculo_status, c.resultado_json, c.parametros_json, c.evidencia_json
...
router.post('/precificacao/itens/:id/calcular', async (req,res) => {
  try {
    ...
    if(dadosBase) resultado.memoria_base=dadosBase.memoria_base;
...
const TIPOS_LOTE_QUESTOR = new Set(['DOCUMENTOS_FISCAIS_CANCELADOS', 'CONCILIAR_CFOP_SAIDAS', 'IMPORTAR_OUTRAS_RECEITAS_LOCACAO']);
const payloadLoteQuestor = (tipo, codigoEmpresa, inicio, fim) => {
  if (tipo === 'DOCUMENTOS_FISCAIS_CANCELADOS') return { actionName:'nFisRRDocFiscalCancelado', parametros:{
    PCODIGOEMPRESA:codigoEmpresa, PDATAINICIAL:inicio, PDATAFINAL:fim, PTIPOMOVIMENTO:2, PTIPOSITUACAODOCUMENTO:2, PORDENAR:1,
  }};
  if (tipo === 'CONCILIAR_CFOP_SAIDAS') return { actionName:'nFisRRResumoConfLctoFisSai', parametros:{
    PCODIGOEMPRESA:codigoEmpresa, PDATAINICIAL:inicio, PDATAFINAL:fim, PTIPOPERIODO:'1', PTIPOIMPOSTO:'1', PTOTALIZAR:'0',
    PLISTAROUTRASINFO:'0', PLISTARTOTALIMPOSTO:'0', PEXIBIRDADOSNATUREZA:'1', PEXIBIRDADOSPESSOA:'1', PEXIBIRDADOSPRODUTO:'0', PLINHAHORIZONTAL:'0', PORDENACAO:'1',
  }};
  if (tipo === 'IMPORTAR_OUTRAS_RECEITAS_LOCACAO') return { actionName:'nFisRRResumoConfLctoFisSaiGrafico', parametros:{
    PCODIGOEMPRESA:codigoEmpresa, PDATAINICIAL:inicio, PDATAFINAL:fim, PTIPOPERIODO:'1', PTIPOIMPOSTO:'2;6', PTOTALIZAR:'0',
    PLISTAROUTRASINFO:'0', PLISTARTOTALIMPOSTO:'0', PEXIBIRDADOSNATUREZA:'1', PEXIBIRDADOSPESSOA:'1', PEXIBIRDADOSPRODUTO:'1', PLINHAHORIZONTAL:'0', PESPECIE:'REC', PORDENACAO:'1',
  }};
  throw new Error('Tipo de busca em lote não suportado.');
};
...
// Uma solicitação em lote é propositalmente desdobrada em tarefas individuais:
// o nWeb recebe o código de cada estabelecimento, o conector mantém execução
// serial e o usuário consegue auditar êxito, ausência de dados ou falha por empresa.
router.post('/questor/lotes', async (req,res)=>{ try {
  const tipo=String(req.body?.tipo||'');
  const inicio=String(req.body?.inicio||''), fim=String(req.body?.fim||'');
  const empresaIds=[...new Set((Array.isArray(req.body?.empresa_ids)?req.body.empresa_ids:[]).map(Number).filter(Number.isInteger))];
  if(!TIPOS_LOTE_QUESTOR.has(tipo)) throw new Error('Selecione uma busca disponível para execução em lote.');
  if(!empresaIds.length) throw new Error('Selecione ao menos uma empresa.');
  if(!/^\d{4}-\d{2}-\d{2}$/.test(inicio)||!/^\d{4}-\d{2}-\d{2}$/.test(fim)||inicio>fim) throw new Error('Informe datas inicial e final válidas.');
  await questorPersistencia.sincronizarUsuario(donoConector(req));
  const conector=db.prepare("SELECT id FROM questor_conectores WHERE status='ATIVO' AND usuario_id=? ORDER BY ultima_conexao_em DESC LIMIT 1").get(donoConector(req));
  if(!conector) throw new Error('Inicie um Conector Sattva–Questor antes de solicitar o lote.');
  const empresas=db.prepare(`SELECT id,razao_social,codigo_questor FROM empresas WHERE id IN (${empresaIds.map(()=>'?').join(',')})`).all(...empresaIds);
  const porId=new Map(empresas.map((e)=>[e.id,e])); const tarefas=[], reaproveitadas=[], ignoradas=[];
  for(const empresaId of empresaIds) {
    const empresa=porId.get(empresaId);
    if(!empresa) { ignoradas.push({empresa_id:empresaId,motivo:'Empresa não encontrada.'}); continue; }
    if(!empresa.codigo_questor) { ignoradas.push({empresa_id:empresa.id,empresa:empresa.razao_social,motivo:'Código Questor não informado.'}); continue; }
    const existente=db.prepare(`SELECT id FROM questor_conector_tarefas
      WHERE conector_id=? AND empresa_id=? AND tipo=? AND status IN ('PENDENTE','EM_EXECUCAO') LIMIT 1`).get(conector.id,empresa.id,tipo);
    if(existente) { reaproveitadas.push({empresa_id:empresa.id,empresa:empresa.razao_social,tarefa_id:existente.id}); continue; }
    const payload=payloadLoteQuestor(tipo,empresa.codigo_questor,inicio,fim);
    const r=db.prepare('INSERT INTO questor_conector_tarefas (conector_id,empresa_id,tipo,payload_json) VALUES (?,?,?,?)')
      .run(conector.id,empresa.id,tipo,JSON.stringify({...payload,lote:{inicio,fim,solicitado_em:new Date().toISOString()}}));
    const tarefa=db.prepare('SELECT * FROM questor_conector_tarefas WHERE id=?').get(r.lastInsertRowid);
    await questorPersistencia.publicarTarefa(tarefa);
    tarefas.push({empresa_id:empresa.id,empresa:empresa.razao_social,tarefa_id:tarefa.id});
  }
...
router.post('/empresas/:id/questor/conector/outras-receitas-locacao', async (req,res)=>{ try {
  ...
  const periodo=await periodoAnalisado.sincronizarCompartilhado(empresaId); if(!periodo) throw new Error('Defina o Período analisado antes da busca.');
  const inicio=String(req.body?.inicio||periodo.data_inicio||''), fim=String(req.body?.fim||periodo.data_fim||'');
  if(!/^\d{4}-\d{2}-\d{2}$/.test(inicio)||!/^\d{4}-\d{2}-\d{2}$/.test(fim)||inicio>fim) throw new Error('Informe datas inicial e final válidas.');
  await questorPersistencia.sincronizarUsuario(donoConector(req));
...
router.post('/empresas/:id/questor/conector/parametros-conferencia-saidas', async (req,res)=>{ try {
  const empresaId=Number(req.params.id), empresa=db.prepare('SELECT codigo_questor FROM empresas WHERE id=?').get(empresaId);
  if(!empresa?.codigo_questor) throw new Error('Informe o Código Questor no cadastro da empresa antes da consulta.');
  const c=db.prepare("SELECT id FROM questor_conectores WHERE status='ATIVO' AND usuario_id=? ORDER BY ultima_conexao_em DESC LIMIT 1").get(donoConector(req));
  if(!c) throw new Error('Inicie um Conector Sattva–Questor antes da consulta.');
  const r=db.prepare("INSERT INTO questor_conector_tarefas (conector_id,empresa_id,tipo,payload_json) VALUES (?,?,?,?)").run(c.id,empresaId,'PARAMETROS_RELATORIO',JSON.stringify({actionName:'nFisRRResumoConfLctoFisSai'}));
  ok(res,{tarefa_id:r.lastInsertRowid});
}catch(e){erro(res,e);}});

router.post('/empresas/:id/questor/conector/conciliar-cfop-saidas', async (req,res)=>{ try {
  const empresaId=Number(req.params.id), empresa=db.prepare('SELECT codigo_questor FROM empresas WHERE id=?').get(empresaId);
  if(!empresa?.codigo_questor) throw new Error('Informe o Código Questor no cadastro da empresa antes da conciliação.');
  const inicio=String(req.body?.inicio||''), fim=String(req.body?.fim||'');
  if(!/^\d{4}-\d{2}-\d{2}$/.test(inicio)||!/^\d{4}-\d{2}-\d{2}$/.test(fim)||inicio>fim) throw new Error('Informe data inicial e final válidas.');
  const c=db.prepare("SELECT id FROM questor_conectores WHERE status='ATIVO' AND usuario_id=? ORDER BY ultima_conexao_em DESC LIMIT 1").get(donoConector(req));
...
router.get('/empresas/:id/motor/status', async (req, res) => {
  try {
    const job = await motorExecucaoFila.status(Number(req.params.id));
