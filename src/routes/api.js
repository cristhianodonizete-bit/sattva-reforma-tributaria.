...
router.get('/questor/conectores/:id/tarefas', (req,res)=>{ try {
...
router.get('/questor/tarefas', (req,res)=>{ try {
...
router.post('/empresas/:id/questor/conector/apuracao-pis-cofins', async (req,res)=>{ try { const empresa=db.prepare('SELECT codigo_questor FROM empresas WHERE id=?').get(req.params.id); if(!empresa?.codigo_questor) throw new Error('Informe o Código Questor no cadastro da empresa antes da busca.'); const periodo=await periodoAnalisado.sincronizarCompartilhado(Number(req.params.id)); if(!periodo) throw new Error('Defina o Período analisado antes da busca.'); const janela=periodoAnalisado.janelaApuracao(periodo); const c=db.prepare("SELECT id FROM questor_conectores WHERE status='ATIVO' AND usuario_id=? ORDER BY ultima_conexao_em DESC LIMIT 1").get(donoConector(req)); if(!c) throw new Error('Inicie um conector Questor que pertença ao seu usuário antes da busca.'); const r=db.prepare("INSERT INTO questor_conector_tarefas (conector_id,empresa_id,tipo,payload_json) VALUES (?,?,?,?)").run(c.id,Number(req.params.id),'APURACAO_PIS_COFINS',JSON.stringify({actionName:'nFisRRTotalPISCOFINSProd',parametros:{codigoempresa:empresa.codigo_questor,datainicial:janela.data_inicio,datafinal:janela.data_fim}})); ok(res,{tarefa_id:r.lastInsertRowid,periodo:janela}); } catch(e){erro(res,e);} });
