...
router.post('/empresas/:id/precificacao/base-operacional/importar', upload.single('arquivo'), async (req,res) => {
  try {
    ...
    const normalizarCnpj=x=>String(x||'').replace(/\D/g,''); const parceiro=db.prepare('SELECT regime FROM parceiros WHERE empresa_id=? AND cnpj=? LIMIT 1'); const cnpjs=[...bases,...estrutura].map(x=>normalizarCnpj(x.fornecedor_cnpj)).filter(Boolean); const compartilhados=await baseRegime.consultarCompartilhada(cnpjs,2024); const resolverRegime=cnpj=>parceiro.get(empresaId,cnpj)?.regime||compartilhados.get(cnpj)?.regime||compartilhados.get(cnpj.slice(0,8))?.regime||baseRegime.consultar(cnpj,2024)?.regime||'regime_regular'; const upsert=db.prepare(`INSERT INTO pricing_base_operacional (empresa_id,codigo,descricao,modelo,ncm_compra,nbs_compra,lc116_compra,ncm_venda,nbs_venda,lc116_venda,fornecedor_cnpj,regime_fornecedor,pis,cofins,valor_aquisicao,prazo_depreciacao_meses,valor_residual,evidencia) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(empresa_id,codigo,modelo) DO UPDATE SET descricao=excluded.descricao,ncm_compra=excluded.ncm_compra,nbs_compra=excluded.nbs_compra,lc116_compra=excluded.lc116_compra,ncm_venda=excluded.ncm_venda,nbs_venda=excluded.nbs_venda,lc116_venda=excluded.lc116_venda,fornecedor_cnpj=excluded.fornecedor_cnpj,regime_fornecedor=excluded.regime_fornecedor,pis=excluded.pis,cofins=excluded.cofins,valor_aquisicao=excluded.valor_aquisicao,prazo_depreciacao_meses=excluded.prazo_depreciacao_meses,valor_residual=excluded.valor_residual,evidencia=excluded.evidencia,atualizado_em=datetime('now','localtime')`);
    db.transaction(()=>{const ids=new Map();bases.forEach(x=>{const cnpj=normalizarCnpj(x.fornecedor_cnpj);upsert.run(empresaId,x.codigo,x.descricao,String(x.modelo).toUpperCase(),x.ncm_compra||'',x.nbs_compra||'',x.lc116_compra||'',x.ncm_venda||'',x.nbs_venda||'',x.lc116_venda||'',cnpj,resolverRegime(cnpj),Number(x.pis)||0,Number(x.cofins)||0,Number(x.valor_aquisicao)||0,Number(x.prazo_depreciacao_meses)||null,Number(x.valor_residual)||0,x.evidencia||'');const id=db.prepare('SELECT id FROM pricing_base_operacional WHERE empresa_id=? AND codigo=? AND modelo=?').get(empresaId,x.codigo,String(x.modelo).toUpperCase()).id;ids.set(x.codigo,id);});const ins=db.prepare('INSERT INTO pricing_base_estrutura (base_operacional_id,codigo_componente,descricao,tipo_componente,valor,pis,cofins,fornecedor_cnpj,regime_fornecedor,ncm,nbs,lc116,evidencia) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)');for(const id of ids.values())db.prepare('DELETE FROM pricing_base_estrutura WHERE base_operacional_id=?').run(id);estrutura.forEach(x=>{const cnpj=normalizarCnpj(x.fornecedor_cnpj);ins.run(ids.get(x.codigo_item),x.codigo_componente,x.descricao,x.tipo_componente,Number(x.valor)||0,Number(x.pis)||0,Number(x.cofins)||0,cnpj,resolverRegime(cnpj),x.ncm||'',x.nbs||'',x.lc116||'',x.evidencia||'');});})(); await require('../services/operacaoCompartilhada').publicar();ok(res,{importado:true,bases:bases.length,componentes:estrutura.length});
  }catch(e){erro(res,e);}
});
