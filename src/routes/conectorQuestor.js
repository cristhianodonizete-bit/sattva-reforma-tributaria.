...
router.post('/poll',async(req,res)=>{try{const c=await autenticar(req);const t=db.prepare("SELECT * FROM questor_conector_tarefas WHERE conector_id=? AND status='PENDENTE' ORDER BY id LIMIT 1").get(c.id);if(t){db.prepare("UPDATE questor_conector_tarefas SET status='EM_EXECUCAO' WHERE id=?").run(t.id);await persistencia.publicarTarefa(db.prepare('SELECT * FROM questor_conector_tarefas WHERE id=?').get(t.id));}res.json({ok:true,tarefa:t?{...t,payload:JSON.parse(t.payload_json||'{}')}:null});}catch(e){res.status(401).json({ok:false,erro:e.message});}});
