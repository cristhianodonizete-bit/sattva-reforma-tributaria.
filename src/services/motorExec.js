...
function pendentesIncrementais(empresaId, opcoes = {}) {
  ...
  if (opcoes.forcar) return db.prepare("SELECT id FROM movimentos WHERE empresa_id=? AND COALESCE(situacao_documento,'AUTORIZADO') NOT IN ('CANCELADO','DENEGADO','INUTILIZADO')").all(empresaId).map((m) => m.id);
