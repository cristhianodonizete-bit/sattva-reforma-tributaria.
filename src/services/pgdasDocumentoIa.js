...
function calcularCompetenciaPisCofins(db,empresaId,valores,validacao){
  ...
  // como memória de conferência — marcado como divergente, nunca transferido
  // para substituir a declaração.
  if(!validacao?.blocos?.length)return null;
  const blocosDeterminaveis=validacao.blocos.every((b)=>b.anexo&&Number.isFinite(Number(b.calculation?.pis_effective_rate))&&Number.isFinite(Number(b.calculation?.cofins_effective_rate)));
  if(!blocosDeterminaveis)return {status:'REVIEW_REQUIRED',pis:null,cofins:null,motivo:validacao.motivo||'O PGDAS não possui regra determinável para todos os blocos.' ,memoria:[]};
  ...
  if(!memoria.length){
    const exemplos=movs.slice(0,3).map((m)=>{const anexo=anexoDoMovimento(m),modelo=String(m.modelo_documento_fiscal||'documento sem modelo').toUpperCase();const identificador=String(m.documento||m.chave||'sem número');return {modelo,identificador,valor:r2(m.valor),motivo:!anexo?'Sem NCM, NBS/LC 116 ou modelo fiscal com Anexo único no PGDAS.':`Associado ao Anexo ${anexo}, mas não há bloco compatível no PGDAS desta competência.`};});
    return {status:'SEM_DOCUMENTOS_CLASSIFICADOS',pis:null,cofins:null,motivo:movs.length?'Há documentos de saída, mas nenhum pôde ser associado com segurança aos Anexos do PGDAS.':'Não há documento de saída ativo importado nesta competência.',exemplos,memoria:[]};
  }
  const divergente=!validacao.validada;return {status:divergente?'CALCULADO_COM_DIVERGENCIA_PGDAS':'CALCULADO',pis:r2(pis),cofins:r2(cofins),motivo:divergente?'Cálculo por competência disponível, mas o valor declarado no PGDAS diverge da reprodução por bloco.':null,exemplos:[],memoria};
}
...
function confirmar(db,empresaId,id,opcoes={}){const doc=db.prepare('SELECT * FROM pgdas_documentos WHERE id=? AND empresa_id=?').get(id,empresaId);if(!doc)throw new Error('Documento PGDAS não encontrado para a empresa.');const campos=db.prepare('SELECT * FROM pgdas_documento_campos WHERE documento_id=?').all(id),valores=Object.fromEntries(campos.map((x)=>[x.campo,x.valor_extraido===null?null:(x.campo==='competencia'?x.valor_extraido:Number(x.valor_extraido))]));if(!valores.competencia||!Number.isFinite(valores.das))throw new Error('Confirme somente quando competência e valor do DAS estiverem identificados no documento.');let blocos=[];try{blocos=JSON.parse(campos.find((x)=>x.campo==='revenue_blocks')?.valor_extraido||'[]');}catch{throw new Error('Blocos de receita PGDAS inválidos. Reprocesse o documento.');}const validacao=validarRegraBlocos(db,valores,blocos),calculo=calcularCompetenciaPisCofins(db,empresaId,valores,validacao),statusDocumento='VALIDADO_USUARIO';db.transaction(()=>{const existente=db.prepare('SELECT id FROM perfil_tributario WHERE empresa_id=? AND competencia=? ORDER BY id DESC LIMIT 1').get(empresaId,valores.competencia),origem=String(doc.tipo_documento||'').startsWith('INTEGRA_CONTADOR')?'pgdas_integra_contador_confirmado':'pgdas_pdf_confirmado';
  // PIS/COFINS do Perfil são a apuração efetivamente declarada no PGDAS, após
  // a reprodução matemática por bloco. Não confundir com o cálculo sobre a
  // receita de competência: este continua nulo quando os buckets de caixa não
  // puderem ser associados aos documentos fiscais com segurança.
  const pisCofinsApurados=validacao.validada?[valores.pis,valores.cofins]:[null,null],perfil=[valores.receita_bruta,valores.receita_recebida,valores.receita_mercadorias,valores.receita_servicos,valores.receita_exportacao,...pisCofinsApurados];if(existente)db.prepare(`UPDATE perfil_tributario SET receita_bruta=COALESCE(?,receita_bruta),receita_recebida=COALESCE(?,receita_recebida),receita_mercadorias=COALESCE(?,receita_mercadorias),receita_servicos=COALESCE(?,receita_servicos),receita_exportacao=COALESCE(?,receita_exportacao),pis=COALESCE(?,pis),cofins=COALESCE(?,cofins),das=?,origem=? WHERE id=?`).run(...perfil,valores.das,origem,existente.id);else db.prepare(`INSERT INTO perfil_tributario (empresa_id,competencia,receita_bruta,receita_recebida,receita_mercadorias,receita_servicos,receita_exportacao,pis,cofins,das,origem) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(empresaId,valores.competencia,...perfil,valores.das,origem);db.prepare("UPDATE pgdas_documento_campos SET status_validacao='VALIDADO_USUARIO' WHERE documento_id=? AND valor_extraido IS NOT NULL").run(id);db.prepare("UPDATE pgdas_documentos SET status_processamento=? WHERE id=?").run(statusDocumento,id);})();return {...listar(db,empresaId).find((x)=>x.id===Number(id)),validacao_regra_simples:validacao,calculo_competencia:calculo};}
function materializarConfirmados(db,empresaId){const documentos=db.prepare("SELECT id FROM pgdas_documentos WHERE empresa_id=? AND status_processamento='VALIDADO_USUARIO' ORDER BY id").all(empresaId);const falhas=[];for(const documento of documentos){try{confirmar(db,empresaId,documento.id,{preservarConfirmacao:true});}catch(e){falhas.push({documento_id:documento.id,erro:e.message});}}return {confirmados:documentos.length,materializados:documentos.length-falhas.length,falhas};}
module.exports={CAMPOS,normalizarTexto,extrairBlocosReceita,findSimplesBracket,calculateSimplesEffectiveRate,getTaxDistribution,calculatePisEffectiveRate,calculateCofinsEffectiveRate,reproducePgdasTaxes,validarRegraBlocos,calcularCompetenciaPisCofins,ingerir,listar,reprocessarCampos,confirmar,materializarConfirmados};
