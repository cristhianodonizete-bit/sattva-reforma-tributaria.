...
function validarRegraBlocos(db,valores,blocos){
  ...
  for(const bloco of blocos){
    ...
    const calculation={faixa:Number(faixa.faixa),rbt12:valores.rbt12,aliquota_nominal:Number(faixa.aliquota_nominal),parcela_deduzir:Number(faixa.parcela_deduzir),aliquota_efetiva_simples:efetiva,pis_distribution_percentage:Number(faixa.rep_pis),cofins_distribution_percentage:Number(faixa.rep_cofins),pis_effective_rate:efetiva*Number(faixa.rep_pis),cofins_effective_rate:efetiva*Number(faixa.rep_cofins)};
    const pgdas_validation={pgdas_pis:pisPgdas,calculated_pis:pisEsperado,pis_delta:pisDelta,pis_match:Math.abs(pisDelta)<=TOLERANCIA_CENTAVOS,pgdas_cofins:cofinsPgdas,calculated_cofins:cofinsEsperado,cofins_delta:cofinsDelta,cofins_match:Math.abs(cofinsDelta)<=TOLERANCIA_CENTAVOS};
    ...
    memoria.push({...bloco,calculation,pgdas_validation,aceite_tributario});
  }
