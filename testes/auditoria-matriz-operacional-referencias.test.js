...
const { auditar, reconciliarNcmsSemReferencia } = require('../src/services/auditoriaMatrizFiscal');
...
db.prepare(`INSERT INTO base_ncm (ncm,descricao) VALUES ('01012100','Válido'),('99999999','Sem referência'),('88888888','Outro sem referência')`).run();
...
assert.equal(resultado.catalogo_operacional.ncm.chaves_sem_referencia, 2);
assert.deepEqual(resultado.catalogo_operacional.ncm.sem_referencia[0], { codigo: '88888888', linhas: 1 });
...
assert.equal(reconciliacao.total, 2);
assert.equal(reconciliacao.itens.find((x) => x.ncm_operacional === '88888888').classificacao, 'SEM_SUCESSOR_OBJETIVO_COM_CONTEXTO');
assert.equal(reconciliacao.itens.find((x) => x.ncm_operacional === '99999999').classificacao, 'CODIGO_INTERNO_OU_TESTE');
assert.equal(reconciliacao.sucessores_oficiais_comprovados, 0);
