...
const questor = apuracoes.importarRelatorioQuestor(db, 2, { resultado:{ Data:`- Saídas\nCódigo da Situação Tributária PIS R$ Valor Receita R$ Base Cálculo R$ PIS\n01 - Tributável 1.000,00 1.000,00 6,50\nCódigo da Situação Tributária COFINS R$ Valor Receita R$ Base Cálculo R$ COFINS\n01 - Tributável 1.000,00 1.000,00 30,00\nTotalização por tipo` } }, { competenciaSolicitada:'2026-09' });
assert.equal(questor.ignorado, false, 'retorno JSON do Questor deve preservar o conteúdo e ser lido');
const questorLido = apuracoes.listarParaRevisao(db, 2).find((x) => x.competencia === '2026-09');
assert.equal(questorLido.receita_base, 1000);
assert.equal(questorLido.pis_debito, 6.5);
assert.equal(questorLido.cofins_debito, 30);
...
assert.match(telaPerfil, /Apurações importadas/);
assert.match(telaPerfil, /Revisar/);
...
assert.match(telaPerfil, /abrirIngestaoApuracao/, 'a tela deve abrir a ingestão sem receber MouseEvent como parâmetro');
db.close();
