...
const salvo = db.prepare('SELECT identificador_origem,item_receita_chave,classificacao_fiscal,criterio_tributacao_atual,cst_motor,cclasstrib_motor FROM receitas_sem_dfe WHERE empresa_id=?').get(empresa);
assert.deepEqual(salvo, { identificador_origem:'38988', item_receita_chave:'LOCACAO_BENS_MOVEIS', classificacao_fiscal:'LOCACAO_BEM_MOVEL', criterio_tributacao_atual:'Não cumulativo: PIS 1,65% + Cofins 7,60%, salvo exceção específica.', cst_motor:'000', cclasstrib_motor:'000001' });
const imovel = db.prepare("SELECT cst,cclasstrib,reducao_cbs,reducao_ibs FROM regras_itens_receita_regime WHERE item_chave='ALUGUEL_IMOVEIS_PROPRIOS' AND regime_empresa='lucro_real'").get();
assert.deepEqual(imovel, { cst:'200', cclasstrib:'200027', reducao_cbs:.7, reducao_ibs:.7 });
