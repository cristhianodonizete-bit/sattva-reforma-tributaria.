...
const supabase = { configurado:() => true, admin:() => ({ from:(tabela) => ({ select:() => ({ order:() => ({ limit:async () => ({
  data:tabela === 'jobs_carteira' ? [{ id:'job-1', status:'PENDENTE', criado_em:new Date().toISOString() }] : [{ worker_id:'worker-principal', status:'ATIVO', heartbeat:new Date().toISOString() }], error:null,
}) }) }) }) }) };
