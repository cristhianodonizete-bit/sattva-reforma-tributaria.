...
const empresaId = db.prepare("INSERT INTO empresas (cnpj,razao_social) VALUES ('11111111000111','Empresa de teste')").run().lastInsertRowid;
db.prepare("INSERT INTO produtos_empresa (empresa_id,codigo_produto_atual,ncm_atual,descricao_atual) VALUES (?,?,?,?)").run(empresaId, 'SKU-1', '55555555', 'Produto histórico');
const porEmpresa = triagemEvidenciasSucessoresHistoricosNcm({ db, empresaId });
assert.equal(porEmpresa.produtos_empresa_identificados, 1);
assert.equal(porEmpresa.itens.find((x) => x.ncm_historico === '55555555').produtos_empresa[0].codigo_produto_atual, 'SKU-1');
